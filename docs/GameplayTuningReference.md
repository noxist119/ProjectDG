...
현재 기본값은 ROUND 1~10을 기록하고, 초반 느린 클리어 기준은 54초, 보스 HP 경고 기준은 30%입니다. 이 값은 ROUND 1~10 실측 2회 전 임시 기준이며, 긴급 지원이 너무 자주 뜨면 `slowEarlyClearSeconds`, `highBossHealthWarningRatio`를 다시 높입니다.
...
계측 결과가 막힘으로 판단되면 ROUND 4~10 사이에 `긴급 지원`을 한 번 엽니다. 긴급 지원은 실패를 무효화하는 장치가 아니라, 사용자가 다시 선택할 힘을 얻도록 돕는 안전망입니다.

- 발동 조건: 라운드 실패, 느린 클리어, 보스 HP 과다 잔존, 1~5라운드 소환/골드 동시 부족
- 노출 횟수: 한 런에 1회
- 상품 수: 기본 3개
- 선택 수: 3개 중 1개, 선택 즉시 긴급 지원 종료
...
- 확인 값: 첫 Rare+ 라운드, 첫 합성 라운드, ROUND 3 부스터 노출/구매율, 긴급 지원 발생/선택률, ROUND 10 보스 HP 잔량
- 추가 확인 값: 운 나쁨 보험 발동률, 보험 선택 후 이탈감, 결과창 `도감`/`상점`/`계속` 버튼 클릭 동선
- 튜닝 판단: 긴급 지원이 2회 테스트 모두 뜨면 초반 난도가 높거나 보상이 부족한 상태입니다. 한 번도 안 뜨고 초반이 밋밋하면 ROUND 3 소형 상점 보상을 더 자극적으로 조정합니다.
...
- Run 1: 첫 Rare+ R1, 첫 합성 R1, ROUND 3 부스터 구매 후 즉시 합성 성공. 긴급 지원을 실제로 선택하지 않은 흐름에서는 R9에서 생명력 0으로 종료했습니다.
- Run 2: 첫 Rare+ R1, 첫 합성 R2, ROUND 3 부스터 구매 후 합성 성공. ROUND 7에서 회복 보급 1회 사용 후 ROUND 10 보스 HP 0%, 생명력 4/20으로 클리어했습니다.
- 판단: 첫 5분 보장 사건 루프는 작동합니다. 현재 핵심 위험은 보상이 아니라 생명력 압박 누적이므로, 긴급 지원은 ROUND 4~10에 1회만 선명하게 노출하고 결과창에서 `다음 판 목표 + 도감 강화 보상`을 강하게 보여줍니다.
...
다음 실측 기준은 최소 20회 R1~R10 플레이입니다. 확인할 값은 첫 Rare+ 라운드, 첫 합성 라운드, ROUND 3 부스터 선택률, 운명 계약 선택률, 긴급지원 선택률, ROUND 10 보스 잔여 HP입니다.

## 21. 2026-06-22 상용화 루프 보강

목표는 기능 수 추가가 아니라 초반 15분의 손맛, 선택의 의미, 반복 복귀 이유를 더 명확히 만드는 것입니다.

- 실측 튜닝 루프: `DefenseGameController.EarlyRunTuningDecisionSummary`가 최근 R1~R10 표본 20회를 기준으로 첫 Rare+, 첫 합성, R10 도달/클리어, R3 상점 구매율, 긴급 지원 선택률, 운명 사용률, R10 보스 HP를 판정합니다.
...
## 22. 2026-08-03 초반 재미·반복 플레이 보강

초반 15분의 행동 이해, 운 실패의 납득, 판별 선택, 매일 재도전 이유를 한 흐름으로 연결했습니다.

- 행동형 10초 안내: 시간뿐 아니라 실제 소환, 라운드 시작, 합성 결과를 따라 `소환 -> 방어 -> 합성 -> 등급 상승` 단계가 진행됩니다.
- 운 보호 장부: 일반 연속 소환, 다음 보호 발동 시점, 보험 사용 여부를 소환 HUD에 상시 표시합니다.
- R10 보스 예고 베팅: 보급/빌드/전술 중 하나를 선택하고 첫 소형 상점 방향을 고정합니다. 폭주에서는 각각 시동 골드 +10, 운 보호 +2칸, 최대 HP +1을 추가로 받으며 목표 적중 시 +45점과 +18G를 받습니다.
- 데일리 운명컵: 날짜 기반 동일 시드와 폭주 모드를 사용하고 증강체·소형 상점 선택 구성을 고정합니다. 날짜별 시도 횟수, 최고 점수, 최고 라운드, 리플레이 요약을 로컬 저장하고 랭킹 화면에 표시합니다. 서버 순위는 아직 연결하지 않았습니다.
- 폭주 억울함 완화: 버스트 물량은 유지하되 일반/호드/보스 체력·공격 배율을 분리하고 라운드당 방어선 피해를 3으로 제한했습니다.

자동 실측 결과:

- Classic R10: 17/20, 85%. 소환 5/7, 균형 6/7, 저축 6/6.
- Overdrive R10 최종: 15/20, 75%. 소환 6/7, 균형 3/7, 저축 6/6.
- 해석: 전체 목표 범위 65~75%의 상단에 도달했지만 전략별 20판 표본 편차가 큽니다. 다음 표본에서는 균형 전략의 운명 카드·초기 합성 성공률과 저축 전략의 생존 우위를 우선 비교합니다.
- 검증: `Assembly-CSharp`와 `Assembly-CSharp-Editor` 경고 0/오류 0, 수직 PlayMode 스모크 pass, 보스 애니메이션 스모크 pass, 런타임 오류 0.

## 23. 2026-08-03 평가 객관성 교정 및 잔여 위험

이전 9.5점은 실제 이용자 지표가 없는 코드·기능 기반 휴리스틱이므로 객관적 절대 점수가 아닙니다. 최대 HP가 재시작 뒤에도 누적되는 결함과 전략별 서로 다른 RNG 시드가 이전 자동 전투 결과에 섞여 있었으므로, 섹션 22의 Overdrive 15/20(75%)는 밸런스 근거로 사용하지 않습니다.

현재 평가는 다음처럼 분리합니다.

- 내부 재미 완성도: 8.2/10, 오차 범위 ±0.7. 초반 15분 루프 30%, 선택 의미 25%, 전투 피드백 20%, 반복 동기 15%, 상용 운영 준비 10%로 가중한 코드·자동 전투 기반 잠정치입니다.
- 상용 검증도: 6.5/10. 실제 신규 이용자의 15분 완주율, 60초 내 재도전율, 선택 분포, D1 유지율이 없으므로 9점대 확정은 금지합니다.
- 객관 점수 승격 조건: 신규 이용자 20~50명 이상, 첫 판 15분 완주율, 패배 후 60초 내 재시작률, 세 전략 선택률/승률, R3 이탈률, D1 유지율을 수집합니다.

교정 및 수정 사항:

- DefenseGameController: 기본 최대 HP를 별도 보존하고 재시작/아웃게임 이동 시 복원해 런 간 HP 누적을 차단했습니다.
- 콘텐츠 시드 오버라이드: 자동 전투에서 같은 시드를 재사용하고, 데일리/테스트 시 이전 런 상점·증강 기록이 개입하지 않게 했습니다.
- Overdrive: 라운드 누수 피해 상한 3에서 2, 빌드 예측 운 보호 +3칸, 보급 예측 레어 유닛 1기와 +10G로 조정했습니다.
- 배치 하네스: 10개 시드마다 세 전략을 모두 실행하고, 시드마다 실행 순서를 회전해 위치 편향을 줄였습니다.
- 회귀 검사: 최대 HP 복원과 동일 시드 난수 시작점 검사를 PlayMode smoke에 추가했습니다.

동일 시드 자동 전투 결과:

- 오염 제거 직후: R10 12/30, 40.0%.
- 생존 보정 후 고정 순서: R10 16/30, 53.3%.
- 보급 보정 후 고정 순서: R10 17/30, 56.7%.
- 실행 순서 회전 최종: R10 15/30, 50.0%. 소환 3/10, 균형 4/10, 상점 8/10.

해석:

- 전체 완주율은 오염 제거 직후보다 +10.0%p 개선됐지만, 상점 중심 80% 대 소환 중심 30%로 선택 우위가 큽니다.
- 동일 코드와 시드에서도 실행 순서에 따라 50.0~56.7%가 나와 자동 전투의 시간 스케줄링 변동이 남아 있습니다. 단일 배치 성공률을 재미 점수로 직접 환산하면 안 됩니다.
- 다음 밸런스 우선순위는 상점 상품 가치/가격과 소환 보드 막힘을 실제 플레이 영상으로 검증하는 것입니다. 봇 승률만 맞추기 위한 추가 너프·버프는 보류합니다.


## 24. 2026-08-03 증강체·전투상점 선택 다양성 보강

재도전할 때 같은 선택지가 반복되는 문제는 보이는 카드 수를 늘리는 대신 후보 풀과 재등장 규칙을 강화하는 방식으로 수정했습니다. 화면에는 비교 가능한 3개 선택지만 유지합니다.

- 재도전 신선도: 일반 런은 같은 라운드의 최근 3회 증강체 ID와 효과 계열, 미니·회복·정규 상점 상품 타입을 우선 제외합니다. 후보가 부족할 때만 단계적으로 제한을 풀어 선택 화면이 비지 않게 합니다.
- 고정 경쟁 모드: 일일 운명컵과 테스트 콘텐츠 시드에서는 기록 기반 제외를 적용하지 않아 같은 시드의 선택 결과가 유지됩니다.
- 보유 영웅 증강체: 자연 노출이 한 번 없었던 경우 다음 증강 선택에서 현재 필드 영웅용 증강체를 최소 1개 보장합니다. 레어·신화 해금 라운드와 이미 선택한 증강체 제한은 유지합니다.
- 신규 증강체 6종: R3 합성 연쇄 엔진, R5 합성 배당, R6 광전사 임계점, R8 보스 전리품 진화·금단 과충전, R9 비전 도미노를 추가했습니다. 합성 횟수, 보스 처치, 저체력 유지, 체력-화력 교환, 스킬 연쇄처럼 플레이 행동이 빌드를 바꾸는 효과입니다.
- 신규 상점 상품 6종: R5 쌍둥이 소환 계약, R6 합성 공방 이용권, R7 치명 개조 키트, R9 마나 공명 도체, R11 에픽 지명 소환·보스 담보 계약을 해금합니다. 기본 상점 일정에 맞춰 R13 전용으로는 두지 않아 실제 플레이에서 항상 접근 가능합니다.
- 상점 풀 크기: 정규 상점 기준 R4 14종, R11 20종, R13 20종입니다. 기존 증강체 상점 상품 3종이 이미 획득되어 제외되더라도 최근 3회 중복을 피할 여유를 확보했습니다.
- 검증: Assembly-CSharp와 Assembly-CSharp-Editor 경고 0/오류 0. DG PlayMode smoke pass, runtimeErrors 0. Choice Variety smoke에서 상점 4회 연속 12개 상품 중복 0, 증강체 ID·효과 계열 차단, 신규 해금 라운드 검사를 모두 통과했습니다.
