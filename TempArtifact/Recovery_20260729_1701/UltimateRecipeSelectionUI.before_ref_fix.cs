		...
		private void RefreshOptionVisuals()
		{
			...
			for (int j = 0; j < optionButtons.Length; j++)
			{
				...
				if (visible)
				{
					...
					((ColorBlock)(ref colors)).normalColor = baseColor;
					((ColorBlock)(ref colors)).highlightedColor = Color.Lerp(baseColor, Color.white, 0.16f);
					((ColorBlock)(ref colors)).selectedColor = Color.Lerp(baseColor, Color.white, 0.22f);
