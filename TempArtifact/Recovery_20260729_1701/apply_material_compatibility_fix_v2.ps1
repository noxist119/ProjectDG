...
Copy-Item -LiteralPath $shaderRelativePath -Destination (Join-Path $backupDirectory 'MobileGpuSkinnedUnit.before_alias_fix_v2.shader')
Copy-Item -LiteralPath $rendererRelativePath -Destination (Join-Path $backupDirectory 'GpuSkinnedUnitBatchRenderer.before_alias_fix_v2.cs')
...
$shaderText = [IO.File]::ReadAllText($shaderPath).Replace("`r`n", "`n")
$shaderText = Replace-Once $shaderText `
    '        _MainTex("Legacy Base Map", 2D) = "white" {}' `
    "        _MainTex(`"Legacy Base Map`", 2D) = `"white`" {}`n        [HideInInspector] _UseLegacyMainTex(`"Use Legacy Main Texture`", Float) = 0" `
    'legacy texture property'
$shaderText = Replace-Once $shaderText `
    '                half4 _EmissionColor;' `
    "                half4 _EmissionColor;`n                half _UseLegacyMainTex;" `
