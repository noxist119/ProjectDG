...
namespace DefenseGame
{
	public class BoardSynergySystem : MonoBehaviour
	{
		...
		private void WireUi()
		{
			if ((Object)(object)summaryButton != null)
			{
				((UnityEvent)(object)summaryButton.onClick).RemoveListener((UnityAction)ToggleExpanded);
				((UnityEvent)(object)summaryButton.onClick).AddListener((UnityAction)ToggleExpanded);
			}
			if ((Object)(object)closeButton != null)
			{
				((UnityEvent)(object)closeButton.onClick).RemoveListener((UnityAction)CloseExpanded);
				((UnityEvent)(object)closeButton.onClick).AddListener((UnityAction)CloseExpanded);
			}
		}
		...
		private void RecalculateSynergies()
		{
			...
			foreach (DefenderUnit defender in defenders)
			{
				if (!(defender == null))
				{
					bonuses[defender] = default(UnitSynergyBonus);
			...
			foreach (DefenderUnit defender2 in defenders)
			{
				if (!(defender2 == null))
				{
					defender2.SetSynergyBonuses(bonuses.TryGetValue(defender2, out var bonus) ? bonus : default(UnitSynergyBonus));
				}
			}
		...
		private void ApplySpecialSynergies(DefenderUnit[] defenders, Dictionary<DefenderUnit, UnitSynergyBonus> bonuses)
		{
			...
			int highGradeCount = GetCount(gradeCounts, CharacterGrade.Legendary) + GetCount(gradeCounts, CharacterGrade.Mythic) + GetCount(gradeCounts, CharacterGrade.Transcendent);
		...
		{
			Color accent = new Color(1f, 0.74f, 0.3f);
			if (count >= 2)
			{
				ApplyToAll(defenders, bonuses, new UnitSynergyBonus
				{
		...
		{
			Color accent = new Color(0.48f, 0.8f, 1f);
			if (count >= 2)
			{
		...
		{
			if ((Object)(object)summaryText != null)
			{
				summaryText.text = ((defenderCount > 0) ? ("시너지 " + activeEntries.Count + "개 활성") : "시너지 대기중");
				((Graphic)summaryText).color = ((defenderCount > 0) ? Color.white : new Color(0.86f, 0.92f, 1f));
			}
			if ((Object)(object)expandedHeaderText != null)
			{
			...
			for (int i = 0; i < rowCount; i++)
			{
				...
				if ((Object)(object)titleTexts[i] != null)
				{
					titleTexts[i].text = (hasEntry ? activeEntries[i].title : (showOverflowRow ? ("+ " + (activeEntries.Count - (rowCount - 1)) + "개 더 있음") : ((i == 0) ? "활성 시너지 없음" : string.Empty)));
					((Graphic)titleTexts[i]).color = ((hasEntry || showOverflowRow) ? Color.white : new Color(0.75f, 0.8f, 0.9f));
				}
				if (detailTexts != null && i < detailTexts.Length && (Object)(object)detailTexts[i] != null)
				{
					detailTexts[i].text = (hasEntry ? activeEntries[i].detail : (showOverflowRow ? "배치가 더 다양해지면 추가 시너지가 표시됩니다." : ((i == 0) ? "같은 역할 또는 같은 속성 태그를 2명 이상 모아보세요." : string.Empty)));
					((Graphic)detailTexts[i]).color = ((hasEntry || showOverflowRow) ? new Color(0.84f, 0.91f, 1f) : new Color(0.58f, 0.66f, 0.84f));
				}
				if (accentImages != null && i < accentImages.Length && (Object)(object)accentImages[i] != null)
				{
					((Graphic)accentImages[i]).color = (hasEntry ? activeEntries[i].color : (showOverflowRow ? new Color(1f, 0.76f, 0.28f, 0.92f) : new Color(0.42f, 0.48f, 0.64f, 0.62f)));
				}
				if (iconImages != null && i < iconImages.Length && (Object)(object)iconImages[i] != null)
				{
					((Graphic)iconImages[i]).color = (hasEntry ? Color.Lerp(activeEntries[i].color, Color.white, 0.38f) : (showOverflowRow ? new Color(1f, 0.84f, 0.38f, 0.7f) : new Color(0.58f, 0.66f, 0.84f, 0.22f)));
				}
			}
		}
		...
		{
			int count;
			return dictionary.TryGetValue(key, out count) ? count : 0;
		}
	}
}
