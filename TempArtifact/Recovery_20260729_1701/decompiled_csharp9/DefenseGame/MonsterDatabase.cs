...
namespace DefenseGame
{
	public class MonsterDatabase : MonoBehaviour
	{
		...
		{
			List<MonsterDefinition> candidates = monsters.Where((MonsterDefinition monster) => monster != null && monster.threatLevel == MonsterThreatLevel.Regular && monster.minRound <= round && monster.grade <= ResolveMaxRegularGrade(round)).ToList();
			if (candidates.Count == 0)
			{
				candidates = monsters.Where((MonsterDefinition monster) => monster != null && monster.threatLevel == MonsterThreatLevel.Regular && monster.minRound <= round).ToList();
			}
			if (candidates.Count == 0)
			{
				candidates = monsters.Where((MonsterDefinition monster) => monster != null && monster.threatLevel == MonsterThreatLevel.Regular).ToList();
			}
		...
		{
			List<MonsterDefinition> candidates = midBosses.Where((MonsterDefinition monster) => monster != null && monster.minRound <= round && monster.grade <= ResolveMaxMidBossGrade(round)).ToList();
			if (candidates.Count == 0)
			{
				candidates = midBosses.Where((MonsterDefinition monster) => monster != null && monster.minRound <= round).ToList();
			}
			if (candidates.Count == 0)
			{
				candidates = midBosses.Where((MonsterDefinition monster) => monster != null).ToList();
			}
			if (candidates.Count == 0)
			{
				return null;
			}
			int index = Mathf.Abs(round + UnityEngine.Random.Range(0, candidates.Count)) % candidates.Count;
		...
		public MonsterDefinition GetBossForRound(int round)
		{
			...
			List<IGrouping<string, MonsterDefinition>> groups = (from monster in bosses
				where monster != null
			...
			List<MonsterDefinition> variants = (from monster in groups[groupIndex]
				where monster.minRound <= round
				orderby monster.variantIndex
				select monster).ToList();
			if (variants.Count == 0)
			{
				variants = groups[groupIndex].OrderBy((MonsterDefinition monster) => monster.variantIndex).ToList();
			}
			MonsterDefinition selected = (from monster in variants
		...
		private MonsterDefinition CreateBossEncounterDefinition(MonsterDefinition source, int round, int encounterIndex)
		{
			...
			definition.rewardGold = Mathf.RoundToInt((float)source.rewardGold * (1f + (float)tier * 0.14f));
			...
			definition.stats.manaRegenPerSecondRate *= Mathf.Lerp(0.88f, 1.24f, Mathf.Clamp01((float)tier / 8f));
			definition.stats.manaGainWhenHitRate *= Mathf.Lerp(0.82f, 1.18f, Mathf.Clamp01((float)tier / 8f));
			definition.stats.manaGainPerAttackRate *= Mathf.Lerp(0.86f, 1.16f, Mathf.Clamp01((float)tier / 8f));
			definition.visualScale = Mathf.Clamp(source.visualScale + (float)tier * 0.012f, 1.58f, 2.05f);
		...
		{
			int overflow;
			BossEncounterBalanceStep balance = ResolveBossEncounterBalance(encounterIndex, out overflow);
		...
		private float ResolveBossEncounterHealthScale(int encounterIndex)
		{
			...
			if (balance != null)
			{
				return Mathf.Max(0.1f, balance.healthScale + (float)overflow * balance.extraHealthScalePerEncounter);
			}
			if (encounterIndex <= 0)
			{
				return 1.5f;
		...
		private float ResolveBossEncounterAttackScale(int encounterIndex)
		{
			...
			if (balance != null)
			{
				return Mathf.Max(0.1f, balance.attackScale + (float)overflow * balance.extraAttackScalePerEncounter);
			}
			if (encounterIndex <= 0)
			{
				return 1.32f;
		...
		private float ResolveBossEncounterManaScale(int encounterIndex)
		{
			...
			if (balance != null)
			{
				return Mathf.Max(0.1f, balance.manaScale + (float)overflow * balance.extraManaScalePerEncounter);
			}
			if (encounterIndex <= 0)
			{
				return 1.05f;
		...
		private float ResolveBossEncounterSkillPowerScale(int encounterIndex)
		{
			...
			if (balance != null)
			{
				return Mathf.Max(0.1f, balance.skillPowerScale + (float)overflow * balance.extraSkillPowerScalePerEncounter);
			}
			if (encounterIndex <= 0)
			{
				return 1.12f;
		...
		private float ResolveBossSkillCooldownScale(int encounterIndex)
		{
			...
			if (balance != null)
			{
				return Mathf.Clamp(balance.cooldownScale + (float)overflow * balance.extraCooldownScalePerEncounter, 0.7f, 1.45f);
			}
			if (encounterIndex <= 0)
			{
				return 1f;
		...
		private float ResolveBossSkillManaThresholdOffset(int encounterIndex)
		{
			...
			if (balance != null)
			{
				return Mathf.Clamp(balance.manaThresholdOffset + (float)overflow * balance.extraManaThresholdOffsetPerEncounter, -20f, 25f);
			}
			if (encounterIndex <= 0)
			{
				return 0f;
		...
		private void TuneBossSkillForEncounter(SkillDefinition skill, int encounterIndex)
		{
			...
			int tier = Mathf.Max(0, encounterIndex);
			int balanceOverflow;
			BossEncounterBalanceStep balance = ResolveBossEncounterBalance(tier, out balanceOverflow);
			...
			case SkillEffectType.DeathPact:
				if (balance != null && balance.convertDeathPactToDamage)
				{
					...
					skill.power = 1.5f + (float)tier * 0.18f;
					skill.duration = 0f;
					skill.hitCount = 1;
					skill.displayName = balance.convertedDeathPactName;
					skill.description = balance.convertedDeathPactDescription;
					skill.power = balance.convertedDeathPactPower + (float)balanceOverflow * balance.extraConvertedDeathPactPowerPerEncounter;
			...
			case SkillEffectType.MassStun:
				skill.hitCount = Mathf.Min(skill.hitCount, ResolveBossSkillTargetCap(balance, balanceOverflow, balance?.maxMassStunTargets ?? ((tier <= 1) ? 1 : ((tier == 2) ? 2 : 3))));
				skill.duration = Mathf.Min(skill.duration, ResolveBossSkillDurationCap(balance, balanceOverflow, balance?.maxStunDuration ?? (1.05f + (float)tier * 0.16f)));
				skill.cooldown = Mathf.Max(skill.cooldown, balance?.minimumControlCooldown ?? (9.5f - Mathf.Min(1.2f, (float)tier * 0.25f)));
				break;
			case SkillEffectType.Stun:
				skill.duration = Mathf.Min(skill.duration, ResolveBossSkillDurationCap(balance, balanceOverflow, balance?.maxStunDuration ?? (1.25f + (float)tier * 0.18f)));
				skill.cooldown = Mathf.Max(skill.cooldown, balance?.minimumSingleStunCooldown ?? (8f - Mathf.Min(1.2f, (float)tier * 0.2f)));
				break;
			case SkillEffectType.AreaDamage:
				skill.radius = Mathf.Min(skill.radius, ResolveBossSkillRadiusCap(balance, balanceOverflow, balance?.maxAreaRadius ?? (3.2f + (float)tier * 0.16f)));
				skill.power = Mathf.Min(skill.power, ResolveBossSkillPowerCap(balance, balanceOverflow, balance?.maxAreaDamagePower ?? (0.92f + (float)tier * 0.15f)));
				break;
			case SkillEffectType.SummonRush:
				skill.hitCount = Mathf.Min(skill.hitCount, ResolveBossSkillTargetCap(balance, balanceOverflow, balance?.maxSummonRushTargets ?? ((tier <= 1) ? 2 : 3)));
				skill.power = Mathf.Min(skill.power, ResolveBossSkillPowerCap(balance, balanceOverflow, balance?.maxSummonRushPower ?? (0.7f + (float)tier * 0.12f)));
				break;
			case SkillEffectType.ManaBurn:
				skill.hitCount = Mathf.Min(skill.hitCount, ResolveBossSkillTargetCap(balance, balanceOverflow, balance?.maxManaBurnTargets ?? ((tier <= 1) ? 1 : ((tier == 2) ? 2 : 3))));
				skill.power = Mathf.Min(skill.power, ResolveBossSkillPowerCap(balance, balanceOverflow, balance?.maxManaBurnRatio ?? (0.22f + (float)tier * 0.05f)));
				skill.cooldown = Mathf.Max(skill.cooldown, balance?.minimumManaBurnCooldown ?? 9.4f);
				break;
			case SkillEffectType.GoldDrain:
				skill.power = Mathf.Min(skill.power, ResolveBossSkillPowerCap(balance, balanceOverflow, balance?.maxGoldDrain ?? (6f + (float)tier * 2.2f)));
				skill.cooldown = Mathf.Max(skill.cooldown, balance?.minimumGoldDrainCooldown ?? 10.5f);
				break;
			case SkillEffectType.BossFortify:
				skill.power = Mathf.Min(skill.power, ResolveBossSkillPowerCap(balance, balanceOverflow, balance?.maxBossFortifyRatio ?? (0.075f + (float)tier * 0.018f)));
				skill.duration = Mathf.Min(skill.duration, ResolveBossSkillDurationCap(balance, balanceOverflow, balance?.maxBossFortifyDuration ?? (3.2f + (float)tier * 0.35f)));
				break;
			case SkillEffectType.MonsterRally:
				skill.power = Mathf.Min(skill.power, ResolveBossSkillPowerCap(balance, balanceOverflow, balance?.maxRallyPower ?? (0.13f + (float)tier * 0.025f)));
				skill.duration = Mathf.Min(skill.duration, ResolveBossSkillDurationCap(balance, balanceOverflow, balance?.maxRallyDuration ?? (3.4f + (float)tier * 0.38f)));
				break;
			}
		}
		...
		{
			float extra = ((balance != null) ? ((float)overflow * Mathf.Max(0f, balance.extraSkillCapPerEncounter)) : 0f);
		...
		{
			float extra = ((balance != null) ? ((float)overflow * Mathf.Max(0f, balance.extraDurationCapPerEncounter)) : 0f);
		...
		{
			float extra = ((balance != null) ? ((float)overflow * Mathf.Max(0f, balance.extraRadiusCapPerEncounter)) : 0f);
			return Mathf.Max(0.1f, baseValue + extra);
		}
		...
		{
			string[] numerals = new string[10] { "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X" };
		...
		private MonsterDefinition CreateMonster(string name, CharacterGrade grade, int seed)
		{
			...
			definition.description = name + " marches toward the final line as a " + role.ToString() + ".";
			...
			definition.rewardGold = (int)(3 + grade);
			definition.accentColor = ResolveColor(grade, role);
			definition.stats = BuildStats(grade, role, seed, isBoss: false);
			definition.skills = BuildSkills(name, grade, role, MonsterThreatLevel.Regular, seed);
			ApplyRegularSwarmProfile(definition, scaleReward: true);
			return definition;
		}
		...
		private MonsterDefinition CreateBoss(string name, int seed)
		{
			...
			definition.accentColor = new Color(1f, 0.25f + (float)seed * 0.08f, 0.25f);
			definition.visualScale = ResolveVisualScale(MonsterThreatLevel.Boss, grade, MonsterRole.Boss, 0);
			definition.stats = BuildStats(grade, MonsterRole.Boss, seed, isBoss: true);
		...
		private CombatStats BuildMidBossStats(CharacterGrade grade, MonsterRole role, int seed)
		{
			...
			CombatStats stats = BuildStats(grade, role, seed + 12, isBoss: false);
			stats.maxHealth *= balance.midBossHealthMultiplier + (float)seed * balance.midBossHealthMultiplierPerSeed;
			stats.attackPower *= balance.midBossAttackMultiplier + (float)seed * balance.midBossAttackMultiplierPerSeed;
		...
		private CombatStats BuildStats(CharacterGrade grade, MonsterRole role, int seed, bool isBoss)
		{
			...
			stats.maxHealth = balance.baseHealth + (float)grade * balance.healthPerGrade + (float)seed * balance.healthPerSeed;
			stats.attackPower = balance.baseAttackPower + (float)grade * balance.attackPowerPerGrade;
			stats.criticalChance = Mathf.Clamp01(balance.baseCriticalChance + (float)grade * balance.criticalChancePerGrade);
			stats.criticalDamageMultiplier = balance.baseCriticalDamageMultiplier + (float)grade * balance.criticalDamageMultiplierPerGrade;
			stats.attackSpeed = balance.baseAttackSpeed + (float)grade * balance.attackSpeedPerGrade;
			stats.maxMana = balance.baseMaxMana + (float)grade * balance.maxManaPerGrade;
			stats.attackRange = balance.baseAttackRange + (float)grade * balance.attackRangePerGrade + (float)(seed % 2) * balance.alternatingAttackRangeBonus;
			stats.moveSpeed = balance.baseMoveSpeed + (float)grade * balance.moveSpeedPerGrade;
			...
			if (isBoss)
			{
				stats.maxHealth = balance.bossBaseHealth + (float)seed * balance.bossHealthPerSeed;
				stats.attackPower = balance.bossBaseAttackPower + (float)seed * balance.bossAttackPowerPerSeed;
				stats.criticalChance = balance.bossBaseCriticalChance + (float)seed * balance.bossCriticalChancePerSeed;
				stats.criticalDamageMultiplier = balance.bossCriticalDamageMultiplier;
				stats.attackSpeed = balance.bossBaseAttackSpeed + (float)seed * balance.bossAttackSpeedPerSeed;
				...
				stats.moveSpeed = balance.bossBaseMoveSpeed + (float)seed * balance.bossMoveSpeedPerSeed;
		...
		private void ApplyRegularSwarmProfile(MonsterDefinition definition, bool scaleReward)
		{
			if (definition != null && definition.threatLevel == MonsterThreatLevel.Regular && definition.stats != null)
			{
				...
				float healthMultiplier = Mathf.Clamp(regularHealthMultiplier + (float)gradeIndex * 0.025f + (float)definition.variantIndex * 0.015f, 0.45f, 0.88f);
				float attackMultiplier = Mathf.Clamp(regularAttackMultiplier + (float)gradeIndex * 0.015f, 0.55f, 0.98f);
		...
		private float ResolveVisualScale(MonsterThreatLevel threatLevel, CharacterGrade grade, MonsterRole role, int variantIndex)
		{
			...
			case MonsterThreatLevel.Boss:
				return Mathf.Clamp(1.68f + (float)variant * 0.045f, 1.6f, 1.95f);
			case MonsterThreatLevel.MidBoss:
			{
				float midBossScale = 1.18f + (float)gradeIndex * 0.025f + (float)variant * 0.025f;
				switch (role)
				{
				case MonsterRole.Brute:
					midBossScale += 0.08f;
			...
			default:
			{
				float scale = 0.74f + (float)gradeIndex * 0.025f + (float)variant * 0.012f;
				switch (role)
				{
				case MonsterRole.Brute:
					scale += 0.08f;
		...
		private List<SkillDefinition> BuildSkills(string ownerName, CharacterGrade grade, MonsterRole role, MonsterThreatLevel threatLevel, int seed)
		{
			...
			default:
			{
				List<SkillEffectType> pool = BuildRoleSkillPool(role, isBoss: false);
				...
				for (int i = 0; i < count; i++)
				{
					...
					result.Add(CreateSkill(ownerName, effectType, isBoss: false, i));
				}
				return result;
			}
			}
		...
		private List<SkillDefinition> BuildMidBossSkills(string ownerName, CharacterGrade grade, MonsterRole role, int seed)
		{
			...
			case 0:
				...
				break;
			case 1:
				result.Add(CreateBossSkill(ownerName, SkillEffectType.ManaBurn, "마나 누수", "무작위 유닛 2기의 마나를 태웁니다.", 0.3f, 0f, 0f, 2, 82f, 8f, 0));
			...
			case 2:
				...
				break;
			case 3:
				result.Add(CreateBossSkill(ownerName, SkillEffectType.GoldDrain, "통행세 징수", "보유 골드를 일부 빼앗습니다.", 8f + (float)seed, 0f, 0f, 1, 84f, 10f, 0));
				result.Add(CreateBossSkill(ownerName, SkillEffectType.MoveSpeedBoost, "광란 질주", "잠시 이동 속도가 크게 증가합니다.", 0.55f, 3.5f, 0f, 1, 70f, 8f, 1));
				break;
			case 4:
				result.Add(CreateBossSkill(ownerName, SkillEffectType.MassStun, "서리 봉인", "무작위 유닛 2기를 짧게 묶습니다.", 0f, 1f, 0f, 2, 88f, 9f, 0));
				result.Add(CreateBossSkill(ownerName, SkillEffectType.HealSelf, "빙결 재생", "체력을 일부 회복합니다.", 0.1f, 0f, 0f, 1, 92f, 10f, 1));
				break;
			default:
		...
		private List<SkillDefinition> BuildBossSkills(string ownerName, int seed)
		{
			...
			case 0:
				...
				result.Add(CreateBossSkill(ownerName, SkillEffectType.BossFortify, "암석 방벽", "체력을 10% 회복하고 잠시 공격 속도를 높입니다.", 0.1f, 4.5f, 0f, 1, 95f, 9.5f, 1));
				break;
			case 1:
				...
				break;
			case 2:
				result.Add(CreateBossSkill(ownerName, SkillEffectType.AreaDamage, "운석 낙하", "주변 유닛에게 공격력 125%의 광역 피해를 줍니다.", 1.25f, 0f, 4f, 1, 80f, 9.5f, 0));
			...
			case 5:
				...
				break;
			case 6:
				result.Add(CreateBossSkill(ownerName, SkillEffectType.BossFortify, "보호막 충전", "보스가 체력을 회복하고 잠시 강화됩니다.", 0.1f, 4.4f, 0f, 1, 76f, 9.2f, 0));
				result.Add(CreateBossSkill(ownerName, SkillEffectType.AreaDamage, "압력 폭발", "주변 유닛에게 강한 광역 피해를 줍니다.", 1.12f, 0f, 3.4f, 1, 92f, 8.2f, 1));
				break;
			default:
				result.Add(CreateBossSkill(ownerName, SkillEffectType.Stun, "앞줄 압박", "가장 가까운 유닛을 기절시켜 전열을 무너뜨립니다.", 0f, 2f, 0f, 1, 72f, 7.4f, 0));
		...
		private SkillDefinition CreateSkill(string ownerName, SkillEffectType effectType, bool isBoss, int index)
		{
			...
			skill.hitCount = ((effectType != SkillEffectType.SummonRush) ? 1 : 3);
			switch (effectType)
			{
		...
		private Color ResolveColor(CharacterGrade grade, MonsterRole role)
		{
			...
			case CharacterGrade.Rare:
				color = new Color(0.35f, 0.8f, 0.95f);
				break;
			case CharacterGrade.Epic:
				color = new Color(0.45f, 0.95f, 0.55f);
				break;
			case CharacterGrade.Legendary:
				color = new Color(1f, 0.7f, 0.25f);
				break;
			...
			{
				color *= new Color(0.85f, 0.95f, 1.1f);
			}
			if (role == MonsterRole.Brute)
			{
				color *= new Color(1.1f, 0.9f, 0.9f);
			}
			return color;
		}
		...
		private CharacterGrade ResolveGrade(int index, int totalCount)
		{
			...
			{
				List<StarterGradeDistributionStep> ordered = (from step in starterGradeDistributionSteps
					where step != null
		...
		private void ApplyPresentationRosterForThreat(MonsterThreatLevel threatLevel, List<MonsterDefinition> target)
		{
			...
			for (int i = 0; i < entries.Count; i++)
			{
				...
				CharacterGrade maxGrade = (entry.createGradeVariants ? ((CharacterGrade)Mathf.Max((int)entry.grade, (int)entry.maxVariantGrade)) : entry.grade);
		...
		private MonsterDefinition CreateMonsterFromRoster(MonsterPresentationOverride entry, MonsterThreatLevel threatLevel, int index, CharacterGrade grade, int variantIndex)
		{
			...
			definition.rewardGold = (hasRewardOverride ? Mathf.RoundToInt((float)entry.rewardGoldOverride * (1f + (float)variantIndex * 0.18f)) : ResolveRosterReward(threatLevel, grade, index, variantIndex));
		...
		{
			string baseName = ((!string.IsNullOrWhiteSpace(entry.displayName)) ? entry.displayName : ((!(entry.prefab != null)) ? fallbackId : entry.prefab.name));
		...
		private Color ResolveRosterColor(MonsterThreatLevel threatLevel, CharacterGrade grade, MonsterRole role, int index)
		{
			...
			return (threatLevel == MonsterThreatLevel.MidBoss) ? Color.Lerp(color, new Color(1f, 0.42f, 0.18f), 0.35f) : color;
		}
		...
		private Color ResolveRosterVariantColor(MonsterPresentationOverride entry, MonsterThreatLevel threatLevel, CharacterGrade grade, MonsterRole role, int index, int variantIndex)
		{
			...
			Color color = Color.Lerp(baseColor, gradeColor, threatLevel switch
			{
			...
			if (variantIndex > 0)
			{
				color = Color.Lerp(color, Color.white, Mathf.Min(0.18f, (float)variantIndex * 0.035f));
			}
			return color;
		}
		...
		private void ApplyVariantStatBonus(CombatStats stats, float bonusPerTier, int variantIndex, MonsterThreatLevel threatLevel)
		{
			...
			{
				float bonus = Mathf.Clamp(bonusPerTier, 0f, 0.35f) * (float)variantIndex;
				stats.maxHealth *= 1f + bonus * threatLevel switch
				{
		...
		private MonsterDefinition CreateBossShadowMidBoss(MonsterDefinition boss)
		{
			...
			definition.isBoss = false;
			definition.rewardGold = Mathf.RoundToInt(20f + (float)boss.grade * 5f + (float)boss.variantIndex * 6f);
