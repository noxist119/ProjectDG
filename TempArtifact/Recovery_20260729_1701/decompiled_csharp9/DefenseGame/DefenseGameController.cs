...
namespace DefenseGame
{
	public class DefenseGameController : MonoBehaviour
	{
		...
		public int ReadyUltimateRecipeCount => (boardManager != null) ? boardManager.GetReadyUltimateRecipeOptions(characterDatabase).Length : 0;
		...
		private void Awake()
		{
			...
			{
				monsterDatabase = GetComponent<MonsterDatabase>();
			}
			if (boardManager == null)
			{
				boardManager = GetComponent<DefenseBoardManager>();
			}
		...
		public bool TrySummon()
		{
			...
			CharacterDefinition summon = SelectSummonDefinition(out earlyPitySummon);
			if (summon == null)
			{
				return false;
			}
			if (!boardManager.TrySpawnUnit(summon, defaultUnitPrefab, out var spawnedUnit))
			{
				return false;
			}
			Gold -= cost;
			...
			this.OnUnitSummoned?.Invoke(summon);
			NotifyStateChanged();
			return true;
		}
		...
		public int GetLuckySummonChoiceCost(LuckySummonChoice choice)
		{
			...
			return (choice == LuckySummonChoice.SafeRare) ? Mathf.Max(baseCost, Mathf.CeilToInt((float)baseCost * Mathf.Max(1f, luckySummonSafeCostMultiplier))) : baseCost;
		}

		public bool CanChooseLuckySummon(LuckySummonChoice choice)
		{
			return LuckySummonReady && luckySummonChoiceOpen && !IsCombatInteractionLocked && boardManager != null && characterDatabase != null && EmptySlotCount > 0 && Gold >= GetLuckySummonChoiceCost(choice);
		}
		...
		public bool TryResolveLuckySummonChoice(LuckySummonChoice choice)
		{
			...
			if (summon == null || !boardManager.TrySpawnUnit(summon, defaultUnitPrefab, out var spawnedUnit))
			{
				return false;
			}
			Gold -= cost;
			int refund = (jackpotFailed ? Mathf.Clamp(Mathf.RoundToInt((float)cost * Mathf.Clamp01(luckySummonJackpotRefundRate)), 0, cost) : 0);
			Gold += refund;
			currentSummonBaseCost = Mathf.Min(maxSummonCost, currentSummonBaseCost + ResolveSummonCostIncrease());
			...
			RegisterSummonExcitement(summon, earlyPitySummon: false, spawnedUnit, trackLuckyStreak: false);
			RecordEarlyRoundSummon(summon);
			ResolveUltimateRecipeBingoReward();
			this.OnUnitSummoned?.Invoke(summon);
			...
			default:
				...
				else
				{
					...
					resultColor = new Color(1f, 0.76f, 0.3f);
				}
				break;
			}
			AddRunHighlightCard("행운 소환", resultMessage);
			this.OnBannerRequested?.Invoke(resultMessage, resultColor, 2.4f);
			NotifyStateChanged();
			return true;
		}
		...
		private CharacterDefinition SelectLuckySummonDefinition(LuckySummonChoice choice, out bool jackpotSuccess, out bool jackpotFailed)
		{
			...
			{
			case LuckySummonChoice.MergeLink:
				selected = characterDatabase.GetRandomCharacterByGrade(SelectMergeAssistGrade(), deployableOnly: true);
				break;
			case LuckySummonChoice.SafeRare:
				selected = characterDatabase.GetRandomSummonableCharacter(GetSummonRateRound(), deployableOnly: true);
				if (selected == null || selected.grade < CharacterGrade.Rare)
				{
					selected = characterDatabase.GetRandomCharacterByGrade(CharacterGrade.Rare, deployableOnly: true);
				}
				break;
			default:
				jackpotSuccess = UnityEngine.Random.value < Mathf.Clamp01(luckySummonJackpotEpicChance);
				jackpotFailed = !jackpotSuccess;
				selected = characterDatabase.GetRandomCharacterByGrade(jackpotSuccess ? CharacterGrade.Epic : CharacterGrade.Normal, deployableOnly: true);
				break;
			}
			if (selected == null)
			{
		...
		private int CalculateRoundStartGold()
		{
			...
			int roundIncome = Mathf.FloorToInt((float)round * Mathf.Max(0f, roundStartGoldPerRoundMultiplier));
		...
		public int ApplyFateShopDebtCost(int baseCost)
		{
			...
			float multiplier = 1f + Mathf.Clamp01((float)fateDebt / (float)Mathf.Max(1, maxFateDebt)) * maxFateDebtShopCostPenalty;
			return Mathf.Max(1, Mathf.RoundToInt((float)baseCost * multiplier));
		}
		...
		public bool TryActivateFateSurvival()
		{
			...
			this.OnBannerRequested?.Invoke("빚지고 살기  생명 +" + recovered + " / 골드 +" + gainedGold, new Color(1f, 0.62f, 0.22f), 2.4f);
			NotifyStateChanged();
			return true;
		}
		...
		{
			if (!TryConsumeFateCard(color: new Color(0.78f, 0.28f, 1f), title: "왕의 공포", detail: "이 라운드 몬스터 체력·공격·이속·공속 " + Mathf.RoundToInt(FateMonsterStatCrushRatio * 100f) + "% 붕괴 / " + BuildFateBacklashText(), debt: Mathf.Max(0, fateCardMonsterCrushDebt)))
			{
				return false;
			}
		...
		{
			if (!TryConsumeFateCard(color: new Color(0.26f, 0.88f, 1f), title: "전장 개방", detail: "이 라운드 전투 중 소환·합성 허용 / 골드 +" + Mathf.Max(0, fateCardCombatGold) + " / HP 절반 지불", debt: Mathf.Max(0, fateCardCombatDraftDebt)))
			{
				return false;
			}
			UnlockFateCombatEditingForCurrentRound();
			Gold += Mathf.Max(0, fateCardCombatGold);
			int lifePaid = ((life > 1) ? Mathf.Max(1, Mathf.FloorToInt((float)life * 0.5f)) : 0);
			if (lifePaid > 0)
			{
		...
		{
			if (!TryConsumeFateCard(color: new Color(1f, 0.36f, 0.24f), title: "피의 계약", detail: "HP 전부 회복 / 모든 유닛 체력 회복 / 다음 보스 강화", debt: Mathf.Max(0, fateCardFullHealDebt)))
			{
				return false;
			}
			life = MaxLife;
		...
		public bool TryActivateFateCardForbiddenSummon()
		{
			...
			if (!TryConsumeFateCard("금단의 소환", "전설 유닛 1개 즉시 획득 / 다음 " + Mathf.Max(1, fateCardForbiddenSummonTaxRounds) + "라운드 소환비 +" + Mathf.RoundToInt(Mathf.Clamp01(fateCardForbiddenSummonCostPenalty) * 100f) + "%", Mathf.Max(0, fateCardForbiddenSummonDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke(granted ? "금단의 소환  전설 유닛 등장" : "금단의 소환  소환 실패", color, 2.4f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardGamblerGold()
		{
			...
			if (!TryConsumeFateCard("도박사의 판", Mathf.RoundToInt(Mathf.Clamp01(fateCardGamblerGoldSuccessRate) * 100f) + "% 확률로 현재 골드만큼 획득하고 전투 편집 / 실패 시 HP -" + Mathf.Max(1, fateCardGamblerFailLifeCost) + "·현재 적 " + Mathf.Max(0.1f, fateCardGamblerFailStunDuration).ToString("0.#") + "초 기절", Mathf.Max(0, fateCardGamblerDebt), color))
			{
				return false;
			}
			if (UnityEngine.Random.value <= Mathf.Clamp01(fateCardGamblerGoldSuccessRate))
			{
				int gainedGold = Mathf.Max(Mathf.Max(0, fateCardGamblerGoldFallbackGain), Gold);
				UnlockFateCombatEditingForCurrentRound();
				Gold += gainedGold;
				AddRunHighlightCard("도박사의 판 성공", "골드 +" + gainedGold);
				this.OnBannerRequested?.Invoke("도박 성공!  골드 +" + gainedGold + " / 전투 편집", color, 2.4f);
			}
			else
			{
				...
				this.OnBannerRequested?.Invoke("도박 실패...  HP -" + lifeCost + " / 적 " + stunned + "기 기절", new Color(1f, 0.36f, 0.24f), 2.4f);
			}
			NotifyStateChanged();
			return true;
		}
		...
		{
			int targetRound = ResolveFateCardTargetRound();
			if (!TryConsumeFateCard(color: new Color(0.45f, 0.78f, 1f), title: "최후의 방벽", detail: "이번 라운드 방어선 돌파 피해 0 / 다음 라운드 적 +" + Mathf.RoundToInt((Mathf.Max(1f, fateCardBacklashMonsterCountMultiplier) - 1f) * 100f) + "%", debt: Mathf.Max(0, fateCardLastBarrierDebt)))
			{
				return false;
			}
		...
		public bool TryActivateFateCardGoldLoan()
		{
			...
			if (!TryConsumeFateCard("황금 대출", "골드 +" + Mathf.Max(0, fateCardGoldLoanGold) + "·이번 라운드 전투 편집 / " + BuildFateBacklashText(), Mathf.Max(0, fateCardGoldLoanDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("황금 대출  골드 +" + Mathf.Max(0, fateCardGoldLoanGold) + " / 전투 편집", color, 2.3f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardRareMercenaries()
		{
			...
			if (!TryConsumeFateCard("용병 호출", "레어 유닛 최대 " + Mathf.Max(1, fateCardRareMercenaryCount) + "개 즉시 획득 / HP -2", Mathf.Max(0, fateCardRareMercenaryDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("용병 호출  레어 +" + granted + " / HP -" + paid, color, 2.3f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardEpicAdvance()
		{
			...
			Color color = new Color(0.76f, 0.4f, 1f);
			if (!TryConsumeFateCard("에픽 선불", "에픽 유닛 1개 + 골드 +" + Mathf.Max(0, fateCardEpicAdvanceGold) + " / 다음 2라운드 소환비 +" + Mathf.RoundToInt(Mathf.Clamp01(fateCardEpicAdvanceCostPenalty) * 100f) + "%", Mathf.Max(0, fateCardEpicAdvanceDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("에픽 선불  에픽 +" + granted, color, 2.4f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardMythicLease()
		{
			...
			Color color = new Color(1f, 0.3f, 0.34f);
			if (!TryConsumeFateCard("신화 임대", "신화 유닛 1개 즉시 획득 / HP가 1로 감소 / 다음 라운드 적 +" + Mathf.RoundToInt((Mathf.Max(1f, fateCardBacklashMonsterCountMultiplier) - 1f) * 100f) + "%", Mathf.Max(0, fateCardMythicLeaseDebt), color))
			{
				return false;
			}
			int targetRound = ResolveFateCardTargetRound();
			...
			this.OnBannerRequested?.Invoke("신화 임대  HP 1, 신화 유닛 등장", color, 2.6f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardBlackMarket()
		{
			...
			if (!TryConsumeFateCard("암시장 개장", "골드 +" + Mathf.Max(0, fateCardBlackMarketGold) + " / 모든 유닛 마나 " + Mathf.RoundToInt(Mathf.Clamp01(fateCardBlackMarketManaRestoreRatio) * 100f) + "% 회복 / 다음 전투상점 확정·가격 상승", Mathf.Max(0, fateCardBlackMarketDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("암시장 개장  " + restored + "기 마나 회복 / 다음 상점 확정", color, 2.3f);
			NotifyStateChanged();
			return true;
		}
		...
		{
			int targetRound = ResolveFateCardTargetRound();
			Color color = new Color(0.48f, 0.9f, 1f);
			...
			if (!TryConsumeFateCard("시간 정지", detail, Mathf.Max(0, fateCardTimeStopDebt), color))
			{
				return false;
			}
			fateTimeStopRound = targetRound;
			int affected = (fateTimeStopAppliedCount = StunActiveMonstersForFate(fateCardTimeStopDuration));
			AddRunHighlightCard("시간 정지", "현재 " + affected + "기 + 이후 등장 전체 기절 / " + backlashText);
			this.OnBannerRequested?.Invoke("시간 정지  현재 " + affected + "기 + 이후 등장 전체", color, 2.3f);
			fateMonsterSurgeRound = targetRound + 1;
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardThunderStrike()
		{
			...
			if (!TryConsumeFateCard("심판 번개", detail, Mathf.Max(0, fateCardThunderDebt), color))
			{
				return false;
			}
			fateThunderStrikeRound = targetRound;
			int affected = (fateThunderStrikeAppliedCount = DamageActiveMonstersForFate(fateCardThunderDamageRatio));
			AddRunHighlightCard("심판 번개", "현재 " + affected + "기 + 이후 등장 전체 타격 / " + backlashText);
			this.OnBannerRequested?.Invoke("심판 번개  현재 " + affected + "기 + 이후 등장 전체", color, 2.3f);
			fateMonsterSurgeRound = targetRound + 1;
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardManaFlood()
		{
			...
			if (!TryConsumeFateCard("마나 폭주", "모든 유닛 마나 회복 / HP -" + Mathf.Max(1, fateCardManaFloodLifeCost), Mathf.Max(0, fateCardManaFloodDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("마나 폭주  " + restored + "기 충전", color, 2.3f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardWallRepair()
		{
			...
			if (!TryConsumeFateCard("응급 방벽", "HP +" + Mathf.Max(1, fateCardWallRepairLife) + " / 모든 유닛 회복 / 다음 2라운드 소환비 +" + Mathf.RoundToInt(Mathf.Clamp01(fateCardWallRepairCostPenalty) * 100f) + "%", Mathf.Max(0, fateCardWallRepairDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("응급 방벽  HP +" + recovered, color, 2.3f);
			NotifyStateChanged();
			return true;
		}
		...
		{
			int targetRound = ResolveFateCardTargetRound();
			Color color = new Color(0.3f, 1f, 0.82f);
			if (!TryConsumeFateCard("밀수 루트", "골드 +" + Mathf.Max(0, fateCardSmugglerRouteGold) + "·이번 라운드 전투 편집 / 이번 포함 " + Mathf.Max(1, fateCardSmugglerRouteRounds) + "라운드 소환비 -" + Mathf.RoundToInt(Mathf.Clamp01(fateCardSmugglerRouteDiscount) * 100f) + "% / " + BuildFateBacklashText(), Mathf.Max(0, fateCardSmugglerRouteDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("밀수 루트  골드 +" + Mathf.Max(0, fateCardSmugglerRouteGold) + " / 전투 편집", color, 2.3f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardLifeForge()
		{
			...
			if (!TryConsumeFateCard("생명 주조", "최대 HP +" + Mathf.Max(1, fateCardLifeForgeMaxLife) + "·모든 유닛 전부 회복 / 현재 골드 절반 소모", Mathf.Max(0, fateCardLifeForgeDebt), color))
			{
				return false;
			}
			int oldGold = Gold;
			int removedGold = Mathf.FloorToInt((float)oldGold * 0.5f);
			...
			this.OnBannerRequested?.Invoke("생명 주조  최대HP +" + lifeGain + " / 유닛 전부 회복", color, 2.4f);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryActivateFateCardGradeRigging()
		{
			...
			if (!TryConsumeFateCard("등급 조작", "골드 +" + Mathf.Max(0, fateCardGradeRiggingGold) + "·이번 라운드 전투 편집 / 다음 " + Mathf.Max(1, fateCardGradeRiggingSummons) + "회 소환 최소 레어 / " + BuildFateBacklashText(), Mathf.Max(0, fateCardGradeRiggingDebt), color))
			{
				return false;
			}
			...
			this.OnBannerRequested?.Invoke("등급 조작  골드 +" + Mathf.Max(0, fateCardGradeRiggingGold) + " / Rare+ " + fateGradeLockSummonsRemaining + "회", color, 2.4f);
			NotifyStateChanged();
			return true;
		}
		...
		private int DamageActiveMonstersForFate(float maxHealthRatio)
		{
			...
			for (int i = 0; i < monsters.Count; i++)
			{
				...
				if (!(monster == null) && monster.CanBeCombatTargeted)
				{
					monster.TakeDamage(monster.MaxHealth * ratio, critical: true, null);
					affected++;
				}
			}
			return affected;
		...
		{
			DefenderUnit[] defenders = ((boardManager != null) ? boardManager.GetAliveDefenders() : UnityEngine.Object.FindObjectsOfType<DefenderUnit>());
			int restored = 0;
		...
		private bool TrySpendFateGauge(int cost, int debt, string title, string detail)
		{
			...
			{
				this.OnBannerRequested?.Invoke("운명 부족  " + fateGauge + "/" + safeCost, new Color(1f, 0.42f, 0.3f), 1.8f);
				return false;
			}
		...
		public void AddTemporaryShopSummonDiscount(float rate, int roundCount)
		{
			...
			temporaryShopSummonDiscountUntilRound = Mathf.Max(temporaryShopSummonDiscountUntilRound, firstDiscountRound + safeRoundCount - 1);
			NotifyStateChanged();
		}
		...
		private float ResolveFateDebtBossHealthMultiplier()
		{
			...
			return 1f + Mathf.Clamp01((float)effectiveDebt / (float)Mathf.Max(1, maxFateDebt)) * maxFateDebtBossHealthBonus;
		}
		...
		public int GetUnitSellRefund(DefenderUnit unit)
		{
			...
			return Mathf.Max(1, Mathf.RoundToInt((float)baseValue * Mathf.Clamp(unitSellRefundRate, 0.1f, 0.5f)));
		}
		...
		public string GetUnitSellDetail(DefenderUnit unit)
		{
			...
			string detail = CharacterGradeUtility.GetDisplayName(unit.Grade) + " " + unit.Definition.displayName + "  |  판매가 " + GetUnitSellRefund(unit) + "G";
		...
		public bool TrySellUnit(DefenderUnit unit, out int refund, out string message)
		{
			...
			string unitName = ((unit.Definition != null && !string.IsNullOrWhiteSpace(unit.Definition.displayName)) ? unit.Definition.displayName : "유닛");
		...
		public bool TryGrantRandomUnitByGrade(CharacterGrade grade)
		{
			...
			CharacterDefinition definition = characterDatabase.GetRandomCharacterByGrade(grade, deployableOnly: true);
			if (definition == null)
			{
				definition = characterDatabase.GetRandomSummonableCharacter(GetSummonRateRound(), deployableOnly: true);
			}
			if (definition == null || !boardManager.TrySpawnUnit(definition, defaultUnitPrefab, out var spawnedUnit))
			{
				return false;
			}
			RegisterGrantedUnitExcitement(definition, spawnedUnit);
			this.OnUnitSummoned?.Invoke(definition);
			NotifyStateChanged();
			return true;
		}
		...
		public bool TryGrantRandomSummonableUnit()
		{
			...
			CharacterDefinition definition = characterDatabase.GetRandomSummonableCharacter(GetSummonRateRound(), deployableOnly: true);
			if (definition == null || !boardManager.TrySpawnUnit(definition, defaultUnitPrefab, out var spawnedUnit))
			{
				return false;
			}
		...
		public bool TryGrantMergeAssistUnit()
		{
			...
			int missingForMerge = ((ownedBefore <= 0) ? 1 : Mathf.Max(1, 3 - ownedBefore));
			...
			{
				CharacterDefinition definition = characterDatabase.GetRandomCharacterByGrade(targetGrade, deployableOnly: true);
				if (definition == null || !boardManager.TrySpawnUnit(definition, defaultUnitPrefab, out var spawnedUnit))
				{
					break;
				}
			...
			this.OnBannerRequested?.Invoke(mergeReady ? ("합성 준비 완료!  " + gradeLabel + " 재료 " + (ownedBefore + granted) + "/3") : ("합성 재료 보급  " + gradeLabel + " +" + granted), CharacterGradeUtility.GetColor(targetGrade, Color.white), mergeReady ? 2.6f : 2.1f);
			NotifyStateChanged();
			return true;
		}
		...
		{
			return (boardManager != null) ? boardManager.GetReadyUltimateRecipeOptions(characterDatabase) : new UltimateRecipeOption[0];
		}

		public UltimateRecipeOption[] GetAllUltimateRecipeOptions()
		{
			return (boardManager != null) ? boardManager.GetAllUltimateRecipeOptions(characterDatabase) : new UltimateRecipeOption[0];
		}
		...
		private void ResolveUltimateRecipeBingoReward()
		{
			...
			foreach (string recipeName in readyRecipes)
			{
		...
		private void HandleMonsterKilled(MonsterUnit monster)
		{
			...
			{
				totalBossKills++;
				string bossName = ((monster.Definition != null && !string.IsNullOrWhiteSpace(monster.Definition.displayName)) ? monster.Definition.displayName : "보스");
		...
		private void HandleMonsterEscaped(MonsterUnit monster)
		{
			...
			if (leakDamage > 0)
			{
				...
				this.OnBannerRequested?.Invoke("방어선 돌파!  HP -" + leakDamage, new Color(1f, 0.38f, 0.24f), 1.8f);
			}
			NotifyStateChanged();
			if (life <= 0)
			{
		...
		private bool ShouldResolveSimultaneousDeathAsVictory()
		{
			...
			return IsSimultaneousDeathVictory(RoundTargetCount, currentRoundKilledMonsters, fatalMonstersPendingResolution);
		}
		...
		private void CaptureDefeatTimeScale()
		{
			if (!defeatTimeScaleCaptured)
			{
				...
				defeatPreviousTimeScale = ((observedScale <= 0.3f) ? 1f : observedScale);
				defeatPreviousFixedDeltaTime = ((Time.fixedDeltaTime > 0f) ? (Time.fixedDeltaTime * defeatPreviousTimeScale / observedScale) : (0.02f * defeatPreviousTimeScale));
		...
		private void UpdateFateChoiceSlowMotion()
		{
			...
			float targetScale = (Time.timeScale = Mathf.Clamp(fateChoiceTimeScale, 0.02f, 1f));
		...
		private void CaptureFateChoiceSlowMotion()
		{
			...
			{
				float observedScale = (fateChoicePreviousTimeScale = Mathf.Max(0.0001f, Time.timeScale));
		...
		private void HandleRoundStateChanged(int round, bool bossRound, bool running)
		{
			...
			if (!running && boardManager != null)
			{
				...
				int clearReward = (LastRoundClearGoldReward = CalculateRoundClearGold(round));
				Gold += clearReward;
				victoryStreak++;
				...
				int unlockedFrontSlots = boardManager.RefreshSlotLocks(round, playUnlockFeedback: true);
				...
				this.OnBannerRequested?.Invoke("ROUND CLEAR  +" + clearReward + "G", new Color(0.48f, 1f, 0.72f), 2.5f);
				ReportRoundCombatRecap(bossRound);
				if (unlockedFrontSlots > 0)
				{
					int unlockedRound = Mathf.Max(1, round + 1);
					this.OnBannerRequested?.Invoke("ROUND " + unlockedRound + "  전방 배치칸 개방 +" + unlockedFrontSlots + "  |  +" + clearReward + "G", new Color(0.46f, 1f, 0.82f), 3f);
		...
		public void ReleasePostRoundChoiceFlow()
		{
			...
			if (round > 0)
			{
				...
				this.OnRoundShopPhase?.Invoke(round);
				this.OnRoundAugmentChoicePhase?.Invoke(round);
				NotifyStateChanged();
			}
		}
		...
		{
			float now = Time.time;
			currentKillCombo = ((!(now - lastKillTime <= 2.2f)) ? 1 : (currentKillCombo + 1));
			lastKillTime = now;
			...
			if (monster != null && monster.IsBoss)
			{
				...
				this.OnBannerRequested?.Invoke("보스 처치!  +" + rewardGold + "G  |  대응 " + responseDamage, color, 2.8f);
				RuntimeAudioUtility.PlayJackpotMajor();
				RuntimeGameFeel.PlayJackpotPulse(monster.transform.position, color, (monster.Definition != null && monster.Definition.IsMajorBoss) ? 2.15f : 1.55f, (monster.Definition != null && monster.Definition.IsMajorBoss) ? 0.2f : 0.14f, 0.42f, 0.15f, 0.1f, 3);
		...
		private int ResolveSummonCost()
		{
			...
			float discounted = (float)baseCost * (1f - Mathf.Clamp01(summonCostDiscountRate));
			...
			{
				cost = Mathf.Max(1, Mathf.RoundToInt((float)cost * (1f - Mathf.Clamp01(temporaryShopSummonDiscountRate))));
			}
			if (enableFateIntervention && useOneShotFateCard && fateSummonDiscountUntilRound >= GetSummonRateRound() && fateSummonDiscountRate > 0f)
			{
				cost = Mathf.Max(1, Mathf.RoundToInt((float)cost * (1f - Mathf.Clamp01(fateSummonDiscountRate))));
			}
			if (enableFateIntervention && useOneShotFateCard && fateSummonTaxUntilRound >= GetSummonRateRound() && fateSummonTaxRate > 0f)
			{
				cost = Mathf.Max(1, Mathf.CeilToInt((float)cost * (1f + Mathf.Clamp01(fateSummonTaxRate))));
			}
			return cost;
		}
		...
		private CharacterDefinition SelectSummonDefinition(out bool earlyPitySummon)
		{
			...
			int summonRateRound = GetSummonRateRound();
			CharacterDefinition selected = characterDatabase.GetRandomSummonableCharacter(summonRateRound, deployableOnly: true);
		...
		private CharacterDefinition ApplyFateSummonIntervention(CharacterDefinition selected)
		{
			...
			if (fateGradeLockSummonsRemaining > 0 && result.grade < fateGradeLockMinimum)
			{
				CharacterDefinition lockedGrade = characterDatabase.GetRandomCharacterByGrade(fateGradeLockMinimum, deployableOnly: true);
				if (lockedGrade != null)
				{
					result = lockedGrade;
					manipulated = true;
			...
			{
				CharacterDefinition rareOrBetter = characterDatabase.GetRandomCharacterByGrade(CharacterGrade.Rare, deployableOnly: true) ?? characterDatabase.GetRandomCharacterByGrade(CharacterGrade.Epic, deployableOnly: true) ?? characterDatabase.GetRandomSummonableCharacter(GetSummonRateRound(), deployableOnly: true);
		...
		private void RegisterSummonExcitement(CharacterDefinition summon, bool earlyPitySummon, DefenderUnit spawnedUnit, bool trackLuckyStreak = true)
		{
			...
			{
				int summonRateRound = GetSummonRateRound();
				bool highGrade = summon.grade >= CharacterGrade.Rare;
				if (!highGrade)
				{
				...
				if (enableEarlyRunFunPacing && summonRateRound <= Mathf.Max(1, earlyFunRoundLimit) && !highGrade && earlyPitySummon)
				{
		...
		private void RegisterMergeExcitement(MergeResultInfo mergeResult)
		{
			...
			if (mergeResult.resultGrade >= CharacterGrade.Rare)
			{
				string label = (mergeResult.isFinalMerge ? "초월 완성!" : ((mergeResult.resultGrade < CharacterGrade.Epic) ? ((CurrentRound <= Mathf.Max(1, earlyFunRoundLimit)) ? "첫 레어 합성!" : "레어 합성!") : ((CurrentRound <= Mathf.Max(1, earlyFunRoundLimit)) ? "초반 대박 합성!" : "대박 합성!")));
				bool majorMerge = mergeResult.isFinalMerge || mergeResult.resultGrade >= CharacterGrade.Epic;
				AddRunHighlightCard(TrimHighlightTitle(label), CharacterGradeUtility.GetDisplayName(mergeResult.resultGrade) + " " + mergeResult.resultCharacterName);
				this.OnBannerRequested?.Invoke(label + "  " + CharacterGradeUtility.GetDisplayName(mergeResult.resultGrade) + " " + mergeResult.resultCharacterName, mergeResult.resultColor, ShortenSummonMergePresentation(mergeResult.isFinalMerge ? 3.2f : (majorMerge ? 2.7f : 2.3f)));
				RuntimeGameFeel.ShowJackpotReveal(label, CharacterGradeUtility.GetDisplayName(mergeResult.resultGrade), mergeResult.resultCharacterName, mergeResult.resultColor, mergeResult.isFinalMerge ? "초월 완성 / 전투 판도 변화" : (majorMerge ? "합성 성공 / 전력 상승" : "첫 고점 / 초반 전력 상승"), ShortenSummonMergePresentation(mergeResult.isFinalMerge ? 3f : (majorMerge ? 2.45f : 2.05f)));
				...
				{
					RuntimeCameraShake.Request((mergeResult.resultGrade >= CharacterGrade.Legendary) ? 0.15f : (majorMerge ? 0.105f : 0.075f), majorMerge ? 0.3f : 0.2f);
				}
			}
		}
		...
		private void PlaySummonJackpotPresentation(CharacterDefinition definition, DefenderUnit spawnedUnit, bool earlyPitySummon, bool spawnSoundAlreadyPlayed = false)
		{
			...
			bool visualMajor = ultimate || definition.grade >= CharacterGrade.Epic;
			...
			string prefix = (ultimate ? "초월 소환!" : (visualMajor ? "대박 소환!" : (earlyPitySummon ? "초반 찬스 소환!" : "희귀 소환!")));
			AddRunHighlightCard(TrimHighlightTitle(prefix), gradeName + " " + definition.displayName);
			this.OnBannerRequested?.Invoke(prefix + "  " + gradeName + " " + definition.displayName, definition.accentColor, ShortenSummonMergePresentation(ultimate ? 3.3f : (visualMajor ? 2.8f : 2.35f)));
			...
			Vector3 position = ((spawnedUnit != null) ? spawnedUnit.transform.position : base.transform.position);
		...
		{
			CharacterGrade[] mergeGrades = new CharacterGrade[4]
			{
			...
			for (int j = 0; j < mergeGrades.Length; j++)
			{
				if (CountUnitsOfGrade(mergeGrades[j]) == 1)
				{
					return mergeGrades[j];
				}
			}
		...
		{
			if (!enableFirstBossSummonRushBonus || firstBossSummonRushBonusGranted || !bossRound || round != Mathf.Max(1, firstBossSummonRushRound) || boardManager == null)
			{
				return;
			}
			...
			float summonProgress = Mathf.Clamp01((float)(summonCount - Mathf.Max(1, firstBossSummonRushMinSummons)) / 12f);
			float mergeProgress = Mathf.Clamp01((float)(mergeCount - Mathf.Max(1, firstBossSummonRushMinMerges)) / 6f);
			float intensity = Mathf.Max(summonProgress, mergeProgress);
			float attackBonus = Mathf.Max(0f, firstBossSummonRushAttackBonus);
			float bossDamageBonus = Mathf.Lerp(Mathf.Max(0f, firstBossSummonRushBossDamageBonus), Mathf.Max(firstBossSummonRushBossDamageBonus, firstBossSummonRushMaxBossDamageBonus), intensity);
			DefenderUnit[] defenders = boardManager.GetAliveDefenders();
			int applied = 0;
			for (int i = 0; i < defenders.Length; i++)
			{
			...
			if (applied > 0)
			{
				...
				this.OnBannerRequested?.Invoke("R10 소환 압축  보스 피해 +" + Mathf.RoundToInt(bossDamageBonus * 100f) + "%", new Color(1f, 0.74f, 0.22f), 1.4f);
			}
		}
		...
		private void CompleteEarlyRoundTelemetry(int round, bool bossRound, bool cleared)
		{
			if (enableEarlyRoundTelemetry && round > 0 && round <= Mathf.Max(1, earlyTelemetryRoundLimit))
			{
				...
				TryRecordEarlyRunTuningLog(snapshot, snapshot.round >= 10);
		...
		private void RecordRoundDefeatMoment(int round, bool bossRound)
		{
			if (!runDefeatMomentRecorded)
			{
				...
				float progress = ((target > 0) ? Mathf.Clamp01((float)defeated / (float)target) : 0f);
				if (round >= 8 || progress >= 0.72f || bossRound)
				{
					string title = "R" + Mathf.Max(1, round) + " 아슬아슬 실패";
					string detail = (bossRound ? "보스 재도전 / 운명 생존 먼저" : ("처치 " + defeated + "/" + target + " / 운명 개입 후보"));
		...
		private void TryRecordEarlyRunTuningLog(EarlyRoundTelemetrySnapshot snapshot, bool reachedRound10)
		{
			...
			int reachedRound11 = ResolveEarlyRunTuningReachedRound(snapshot);
			bool recordPartialRun = gameOverRaised && reachedRound11 > 0 && reachedRound11 < 10;
			bool recordReachedRun = reachedRound10 || reachedRound11 >= 10;
			if (recordPartialRun || recordReachedRun)
			{
				...
				else if (recordReachedRun && CurrentRound == 10 && IsBossRound)
				{
				...
				EarlyRunTuningLogEntry entry = new EarlyRunTuningLogEntry
				{
					...
					reachedRound = Mathf.Clamp(reachedRound11, 1, 10),
					reachedRound10 = recordReachedRun,
					clearedRound10 = (recordReachedRun && snapshot != null && snapshot.round >= 10 && snapshot.cleared),
		...
		private void LoadEarlyRunTuningLogStore()
		{
			...
			string json = PlayerPrefs.GetString("DefenseGame.EarlyRunTuningLog.v1", string.Empty);
			if (!string.IsNullOrWhiteSpace(json))
			{
				...
				{
					Debug.LogWarning("[EarlyRunTelemetry] 누적 로그 로드 실패: " + ex.Message);
				}
			}
			if (earlyRunTuningLogStore == null)
			{
		...
		{
			EnsureEarlyRunTuningLogStoreLoaded();
			PlayerPrefs.SetString("DefenseGame.EarlyRunTuningLog.v1", JsonUtility.ToJson(earlyRunTuningLogStore));
			PlayerPrefs.Save();
		}
		...
		private void UpdateEarlyRunLogCoverageSummary()
		{
			...
			if (entryCount <= 0)
			{
				string r10BossHp = ((earlyRunR10BossHealthRemaining01 >= 0f) ? (Mathf.RoundToInt(earlyRunR10BossHealthRemaining01 * 100f) + "%") : "-");
			...
			earlyRunLogCoverageSummary = "R1~R10 로그 " + FormatSampleProgress(entryCount, target) + " / 첫R+ " + FormatAverageRound(firstRareSum, firstRareCount) + " / 첫합 " + FormatAverageRound(firstMergeSum, firstMergeCount) + " / 보험 " + insuranceOfferedCount + "/" + entryCount + " " + FormatRate(insuranceOfferedCount, entryCount) + " / R3부스터 " + r3PurchaseCount + "/" + r3OfferCount + " " + FormatRate(r3PurchaseCount, r3OfferCount) + " / 긴급지원 " + recoveryPurchaseCount + "/" + recoveryOfferCount + " " + FormatRate(recoveryPurchaseCount, recoveryOfferCount) + " / 운명계약 " + fateContractUseCount + "/" + entryCount + " " + FormatRate(fateContractUseCount, entryCount) + " / 운명개입 " + fateInterventionUseCount + "/" + entryCount + " " + FormatRate(fateInterventionUseCount, entryCount) + " / 빚 " + FormatAverageInt(fateDebtSum, entryCount) + " / R10보스HP " + FormatAveragePercent(r10BossHpSum, r10BossHpCount);
		}
		...
		private string BuildEarlyRunTuningDecisionSummary()
		{
			...
			float firstRareAvg = ((firstRareCount > 0) ? ((float)firstRareSum / (float)firstRareCount) : 99f);
			float firstMergeAvg = ((firstMergeCount > 0) ? ((float)firstMergeSum / (float)firstMergeCount) : 99f);
			float reachedRate = ((sampleCount > 0) ? ((float)reachedRound10Count / (float)sampleCount) : 0f);
			float clearRate = ((sampleCount > 0) ? ((float)clearedRound10Count / (float)sampleCount) : 0f);
			float r3PurchaseRate = ((r3OfferCount > 0) ? ((float)r3PurchaseCount / (float)r3OfferCount) : 0f);
			float recoveryPurchaseRate = ((recoveryOfferCount > 0) ? ((float)recoveryPurchaseCount / (float)recoveryOfferCount) : 0f);
			float fateUseRate = ((sampleCount > 0) ? ((float)fateUseCount / (float)sampleCount) : 0f);
			float r10BossHpAvg = ((r10BossHpCount > 0) ? (r10BossHpSum / (float)r10BossHpCount) : 0f);
			if (firstRareAvg > 2f)
			{
		...
		private string BuildEarlyRoundTelemetrySummary(EarlyRoundTelemetrySnapshot snapshot)
		{
			...
			string summary = "R" + snapshot.round + " " + snapshot.clearTimeSeconds.ToString("0") + "s / G" + snapshot.endGold + " / HP " + snapshot.endLife + "/" + MaxLife + " / 소환 " + snapshot.summons + " / 합성 " + mergeText + " / 첫R+ " + FormatEarlyMomentRound(firstRarePlusRound) + " / 첫합 " + FormatEarlyMomentRound(firstMergeRound);
			if (snapshot.bossRound)
			{
				summary = summary + " / 보스HP " + Mathf.RoundToInt(Mathf.Clamp01(snapshot.bossHealthRemaining01) * 100f) + "%";
			}
			return summary;
		}
		...
		{
			float lifeRatio = ((MaxLife > 0) ? Mathf.Clamp01((float)Life / (float)MaxLife) : 0f);
			int score = Mathf.RoundToInt((float)CurrentRound * 12f);
		...
		private List<string> CollectRunResultCards(int targetCount)
		{
			...
			int i = runHighlightCards.Count - 1;
			while (i >= 0 && cards.Count < safeTarget)
			{
				if (!string.IsNullOrWhiteSpace(runHighlightCards[i]))
				{
					cards.Add(runHighlightCards[i]);
		...
		private string BuildRunResultNextCompactSummary()
		{
			...
			string growthAction = ((upgradeableCards > 0) ? ("강화 " + upgradeableCards + "명 가능") : ((earnedGrowthCurrency > 0) ? ("다이아 +" + earnedGrowthCurrency + " 성장") : "카드 조각 목표"));
			string pressureAction = (earlyRunRecoveryRecommended ? "복구 상점" : ((RoundsUntilNextBoss > 0 && RoundsUntilNextBoss <= 2) ? "보스 대비" : "상점 보충"));
		...
		private string BuildFateInterventionSummary()
		{
			...
			string pending = ((fateGradeLockSummonsRemaining > 0) ? (" 잠금 " + CharacterGradeUtility.GetDisplayName(fateGradeLockMinimum) + "x" + fateGradeLockSummonsRemaining) : ((fateNormalBanSummonsRemaining > 0) ? (" 일반금지x" + fateNormalBanSummonsRemaining) : (fateForceNextShop ? " 상점확정" : string.Empty)));
		...
		private string BuildFateResultSummary()
		{
			...
			int nextCost = Mathf.Min(Mathf.Min(Mathf.Max(1, fateShopRerollGaugeCost), Mathf.Max(1, fateNormalBanGaugeCost)), Mathf.Max(1, fateForceShopGaugeCost));
		...
		private string BuildReadableFateInterventionSummary()
		{
			...
			string pending = ((fateGradeLockSummonsRemaining > 0) ? (" / Rare+ " + fateGradeLockSummonsRemaining) : ((fateNormalBanSummonsRemaining > 0) ? (" / 일반 제외 " + fateNormalBanSummonsRemaining) : (fateForceNextShop ? " / 다음 상점 확정" : string.Empty)));
			return "운명 " + fateGauge + "/" + Mathf.Max(1, maxFateGauge) + " | 빚 " + fateDebt + "/" + Mathf.Max(1, maxFateDebt) + pending;
		}
		...
		private string BuildReadableFateResultSummary()
		{
			...
			int nextCost = Mathf.Min(Mathf.Min(Mathf.Max(1, fateNormalBanGaugeCost), Mathf.Max(1, fateForceShopGaugeCost)), Mathf.Max(1, fateSurvivalGaugeCost));
			return (fateGauge >= nextCost) ? "운명 개입 준비" : ("운명 " + fateGauge + "/" + nextCost);
		}
		...
		private string BuildReadableFateHudSummary()
		{
			...
			string pending = ((fateGradeLockSummonsRemaining > 0) ? (" / Rare+ x" + fateGradeLockSummonsRemaining) : ((fateNormalBanSummonsRemaining > 0) ? (" / 일반 제외 x" + fateNormalBanSummonsRemaining) : (fateForceNextShop ? " / 상점 확정" : string.Empty)));
			return "운명 " + fateGauge + "/" + Mathf.Max(1, maxFateGauge) + " | 빚 " + fateDebt + "/" + Mathf.Max(1, maxFateDebt) + pending;
		}
		...
		private string BuildReadableFateCostBenefitSummary()
		{
			...
			int bossPenaltyPercent = Mathf.RoundToInt(Mathf.Clamp01((float)runPeakFateDebt / (float)Mathf.Max(1, maxFateDebt)) * maxFateDebtBossHealthBonus * 100f);
		...
		private string BuildFateHudSummary()
		{
			...
			string pending = ((fateGradeLockSummonsRemaining > 0) ? (" / Rare+ lock x" + fateGradeLockSummonsRemaining) : ((fateNormalBanSummonsRemaining > 0) ? (" / No Normal x" + fateNormalBanSummonsRemaining) : (fateForceNextShop ? " / Shop reserved" : string.Empty)));
			return "운명 " + fateGauge + "/" + Mathf.Max(1, maxFateGauge) + " | 빚 " + fateDebt + "/" + Mathf.Max(1, maxFateDebt) + pending;
		}
		...
		private string BuildSeasonReplayDigestSummary()
		{
			...
			return "협동 보스 " + RunBossScore.ToString("N0") + " / MVP " + RunMvpName + " | 리플레이 " + BuildRunShareCode() + " " + latestMoment;
		}
		...
		private string BuildRunShareCode()
		{
			...
			return "#" + Mathf.Abs(hash % 100000).ToString("D5");
		}
		...
		private static string CompactRunResultText(string value, int maxChars)
		{
			...
			return compact[..Mathf.Max(1, maxChars - 3)] + "...";
		}
		...
		{
			int upgradeableCards = GetUpgradeableCardCount();
			string collectionAction = ((upgradeableCards > 0) ? ("도감 강화: 강화 가능 " + upgradeableCards + "명 / 바로 성장 확인") : ((earnedGrowthCurrency > 0) ? ("도감 강화 보상: 다이아 +" + earnedGrowthCurrency + " / 주력 카드 성장 확인") : "도감 강화 보상: 주력 딜러 카드 조각 목표 확인"));
			string shopAction = (earlyRunRecoveryRecommended ? ("상점 목표: " + earlyRunRecoveryCause + " / " + earlyRunRecoveryReason) : "상점 목표: 부족한 등급과 카드 조각 보충");
			string bossAction = ((RoundsUntilNextBoss > 0 && RoundsUntilNextBoss <= 2) ? ("다음 보스 대비: " + ComposeBuildGoalGuideSummary()) : ((totalBossSkillCasts > 0 && totalBossTileDamage < Mathf.Max(1f, totalBossSkillDamage * 0.35f)) ? "다음 보스 대비: 보스 타일과 군중 제어 우선" : ((!(topDamageHeroDamage <= 0f)) ? ("다음 보스 대비: " + RecommendedDeckSummary) : "다음 보스 대비: 핵심 딜러 먼저 확보")));
		...
		{
			int upgradeableCards = GetUpgradeableCardCount();
			string collectionHook = ((upgradeableCards > 0) ? ("강화 가능 " + upgradeableCards + "명") : ((earnedGrowthCurrency > 0) ? "도감 강화 가능" : "도감 카드 조각 목표"));
			string buildName = ((totalBossKills <= 0 && CurrentRound >= 4) ? "보스 사냥덱" : RecommendedBuildName);
		...
		private string BuildBossPressureSummary(bool total)
		{
			...
			{
				summary = summary + " / 영향 " + affectedTargets;
			}
			if (skillDamage > 0f)
			{
				summary = summary + " / 피해 " + Mathf.RoundToInt(skillDamage).ToString("N0");
			}
			if (goldDrained > 0)
			{
				summary = summary + " / 골드 -" + goldDrained;
			}
			if (manaBurnTargets > 0)
			{
				summary = summary + " / 마나 " + manaBurnTargets;
			}
			if (executions > 0)
			{
				summary = summary + " / 즉사 " + executions;
			}
			if (fortifyCount > 0)
			{
				summary = summary + " / 강화 " + fortifyCount;
			}
			if (rallyTargets > 0)
			{
				summary = summary + " / 집결 " + rallyTargets;
			}
			if (responseDamage > 0f)
			{
				summary = summary + " / 대응 " + Mathf.RoundToInt(responseDamage).ToString("N0");
			}
			return summary;
		}
		...
		private void ReportRoundCombatRecap(bool bossRound)
		{
			if (!(topDamageHeroDamage <= 0f))
			{
				...
				{
					message = message + "  |  타일 " + tileSummary;
				}
				this.OnBannerRequested?.Invoke(message, bossRound ? new Color(1f, 0.72f, 0.26f) : new Color(0.4f, 0.92f, 1f), bossRound ? 2.8f : 2.2f);
			}
		}
		...
		private string BuildCurrentTileContributionSummary()
		{
			...
			{
				summary = summary + " / 보스 " + Mathf.RoundToInt(currentRoundBossTileDamage).ToString("N0");
			}
			return summary;
		}
		...
		private static string BuildDamageLeaderboardSummary(Dictionary<string, float> table, int maxCount)
		{
			...
			entries.Sort((KeyValuePair<string, float> left, KeyValuePair<string, float> right) => right.Value.CompareTo(left.Value));
			...
			for (int i = 0; i < count; i++)
			{
				...
				summary = summary + (i + 1) + ". " + entries[i].Key + " " + Mathf.RoundToInt(entries[i].Value).ToString("N0");
			}
			return summary;
		}
		...
		private void ReportTopDamageFeedback(string heroName, float damage, string previousHeroName, float previousDamage)
		{
			...
			{
				bool leaderChanged = previousDamage > 0f && !string.Equals(heroName, previousHeroName, StringComparison.Ordinal);
				...
				if (leaderChanged || reachedMilestone)
				{
					...
					this.OnBannerRequested?.Invoke((leaderChanged ? "최고 딜러 갱신!  " : "딜러 폭주!  ") + heroName + " " + Mathf.RoundToInt(damage).ToString("N0"), new Color(1f, 0.82f, 0.24f), 1.8f);
				}
			}
		}
		...
		private void ReportRoundTileContribution(bool bossRound)
		{
			...
			if (leadingType != BoardTileModifierType.None)
			{
				...
				{
					this.OnBannerRequested?.Invoke("보스전 배치 적중!  " + tileName + " 보스 피해 " + Mathf.RoundToInt(currentRoundBossTileDamage).ToString("N0"), new Color(1f, 0.7f, 0.22f), 2.6f);
					return;
				}
				this.OnBannerRequested?.Invoke("전술 타일 기여  " + tileName + " " + Mathf.RoundToInt(currentRoundTileDamage).ToString("N0") + " 피해", new Color(0.35f, 0.92f, 1f), 2.2f);
			}
		}
