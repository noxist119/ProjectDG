...
namespace DefenseGame
{
	public class RunShopSystem : MonoBehaviour
	{
		...
		private void WireUi()
		{
			...
			{
				((UnityEvent)(object)closeButton.onClick).RemoveListener((UnityAction)Close);
				((UnityEvent)(object)closeButton.onClick).AddListener((UnityAction)Close);
			}
			if ((Object)(object)reopenButton != null)
			{
				((UnityEvent)(object)reopenButton.onClick).RemoveListener((UnityAction)Open);
				((UnityEvent)(object)reopenButton.onClick).AddListener((UnityAction)Open);
			}
			...
			{
				int index = i;
				if (!((Object)(object)offerButtons[i] == null))
				{
					((UnityEventBase)(object)offerButtons[i].onClick).RemoveAllListeners();
					((UnityEvent)(object)offerButtons[i].onClick).AddListener((UnityAction)delegate
					{
						BuyOffer(index);
					});
				}
		...
		private bool ShouldOpenEarlyMiniShop(int round)
		{
			...
			if (!enableEarlyMiniShop || lastMiniShopRound == round || round < firstRound || !scheduledRound)
			{
				if (scheduledRound)
				{
					earlyMiniShopMisses = 0;
		...
		{
			if (!enableEarlyRecoveryShop || earlyRecoveryShopShown || gameController == null || !gameController.EarlyRunRecoveryRecommended || gameController.BadLuckInsuranceAvailable || round < Mathf.Max(1, earlyRecoveryShopFirstRound) || round > Mathf.Max(earlyRecoveryShopFirstRound, earlyRecoveryShopLastRound))
			{
				return false;
			}
		...
		private bool RollLuckyShopAppearance(float baseChance, int missCount, int guaranteedAfterMisses)
		{
			...
			float chance = Mathf.Min(chanceCeiling, Mathf.Clamp01(baseChance + (float)Mathf.Max(0, missCount) * missedShopChanceBonus));
		...
		private void BuildOffers(int round, bool miniShop, bool recoveryShop, bool insuranceShop)
		{
			...
			List<OfferType> pool = (recoveryShop ? BuildRecoveryShopPool() : (miniShop ? BuildMiniShopPool(round) : BuildRegularShopPool(round)));
		...
		private void RemoveRecentlyOfferedTypes(List<OfferType> pool, int minimumPoolSize)
		{
			...
			int i = pool.Count - 1;
			while (i >= 0 && pool.Count > safeMinimum)
			{
		...
		private static int ResolveFixedMiniShopPrice(OfferType type, int round)
		{
			...
			return type switch
			{
				OfferType.RandomUnit => Mathf.Max(1, target - 1), 
				OfferType.Coupon => Mathf.Max(1, target - 2), 
				OfferType.TileReroll => Mathf.Max(1, target - 4), 
				OfferType.FieldMedic => Mathf.Max(1, target - 3), 
				OfferType.RiskChest => Mathf.Max(1, target - 4), 
				_ => target, 
			};
		}
		...
		private OfferType ResolveInsuranceOfferType(int round)
		{
			...
			float lifeRatio = ((gameController.MaxLife > 0) ? Mathf.Clamp01((float)gameController.Life / (float)gameController.MaxLife) : 1f);
			if (lifeRatio <= 0.55f)
			{
		...
		private void AddOffer(OfferType type, int round, bool miniShop, bool recoveryShop)
		{
			...
			if (offer != null)
			{
				...
				else
				{
					...
					ApplySummonOpportunityCostFloor(offer, miniShop: false, recoveryShop: false);
		...
		private static void ApplyRoundShopInflation(ShopOffer offer, int round)
		{
			if (offer != null && offer.cost > 0)
			{
				float multiplier = ((round >= 21) ? 1.3f : ((round >= 15) ? 1.15f : 1f));
				if (!(multiplier <= 1f))
				{
					offer.cost = Mathf.Max(1, Mathf.CeilToInt((float)offer.cost * multiplier));
		...
		private void ApplySummonOpportunityCostFloor(ShopOffer offer, bool miniShop, bool recoveryShop)
		{
			if (offer != null && offer.cost > 0 && !(gameController == null))
			{
				...
				{
					float shopRate = (miniShop ? miniShopOpportunityRate : (recoveryShop ? recoveryShopOpportunityRate : regularShopOpportunityRate));
					int summonReferenceCost = Mathf.Max(1, gameController.SummonCost);
					int minimumCost = Mathf.CeilToInt((float)summonReferenceCost * summonEquivalent * Mathf.Max(0.1f, shopRate));
		...
		private static void ApplyDailyFortuneOfferModifier(ShopOffer offer)
		{
			if (offer != null && offer.cost > 0)
			{
				...
				{
					offer.cost = Mathf.Max(1, Mathf.RoundToInt((float)offer.cost * fortune.ShopCostMultiplier));
		...
		private bool ApplyOffer(ShopOffer offer)
		{
			...
			case OfferType.RiskChest:
			{
				...
				CharacterGrade grade = ((roll < 0.1f) ? CharacterGrade.Legendary : ((!(roll < 0.38f)) ? CharacterGrade.Rare : CharacterGrade.Epic));
			...
			case OfferType.BossIntel:
			{
				...
				for (int m = 0; m < defenders.Length; m++)
				{
					if (!(defenders[m] == null))
					{
						defenders[m].AddAttackPowerBonus(0.08f);
						defenders[m].AddBossDamageBonus(0.3f);
					}
				}
				return true;
			}
			...
			case OfferType.FateBossContract:
			{
				...
				bool buffed = false;
				for (int j = 0; j < fateBossUnits.Length; j++)
				{
					if (!(fateBossUnits[j] == null))
					{
						fateBossUnits[j].AddBossDamageBonus(0.16f);
						buffed = true;
					}
				}
				if (!buffed)
			...
			case OfferType.RecoveryBossPrep:
			{
				...
				for (int l = 0; l < bossPrepUnits.Length; l++)
				{
					if (!(bossPrepUnits[l] == null))
					{
						bossPrepUnits[l].Heal(bossPrepUnits[l].MaxHealth * 0.3f);
						bossPrepUnits[l].AddAttackPowerBonus(0.05f);
						bossPrepUnits[l].AddBossDamageBonus(0.2f);
					}
				}
				return true;
			}
			...
			case OfferType.FieldMedic:
			{
				...
				{
					units[k]?.Heal(units[k].MaxHealth * 0.25f);
				}
		...
		private void ApplyShopVisualIdentity()
		{
			...
			if (currentShopIsInsurance)
			{
				accent = new Color(0.68f, 0.96f, 0.3f, 1f);
				modalColor = new Color(0.06f, 0.2f, 0.16f, 0.99f);
			...
			else if (currentShopIsRecovery)
			{
				...
				cardColor = new Color(0.27f, 0.16f, 0.1f, 0.99f);
			...
			else if (currentShopIsMini)
			{
				...
				modalColor = new Color(0.04f, 0.2f, 0.25f, 0.99f);
			...
			else
			{
				...
				modalColor = new Color(0.065f, 0.11f, 0.3f, 0.99f);
			...
			if ((Object)(object)modalImage != null)
			{
				((Graphic)modalImage).color = modalColor;
			}
			if ((Object)(object)headerPillImage != null)
			{
				((Graphic)headerPillImage).color = accent;
				((Graphic)headerPillImage).rectTransform.sizeDelta = headerSize;
			}
			...
			if ((Object)(object)subtitleText != null)
			{
				((Graphic)subtitleText).color = Color.Lerp(Color.white, accent, 0.34f);
			}
			if ((Object)(object)reopenButton != null)
			{
				Graphic targetGraphic = ((Selectable)reopenButton).targetGraphic;
				Image reopenImage = (Image)(object)((targetGraphic is Image) ? targetGraphic : null);
				if (reopenImage != null)
				{
					((Graphic)reopenImage).color = accent;
				}
			}
			if ((Object)(object)footerHintText != null)
			{
				((Graphic)footerHintText).color = Color.Lerp(Color.white, accent, 0.3f);
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (!((Object)(object)button == null))
				{
					...
					rect.sizeDelta = size;
					Graphic targetGraphic2 = ((Selectable)button).targetGraphic;
					Image cardImage = (Image)(object)((targetGraphic2 is Image) ? targetGraphic2 : null);
					if (cardImage != null)
					{
						((Graphic)cardImage).color = cardColor;
					}
					Outline outline = ((Component)(object)button).GetComponent<Outline>();
					if ((Object)(object)outline != null)
					{
						((Shadow)outline).effectColor = new Color(accent.r, accent.g, accent.b, 0.96f);
					}
					Image dock = GetChildImage(((Component)(object)button).transform, "PriceDock");
					if ((Object)(object)dock != null)
					{
						((Graphic)dock).color = dockColor;
					}
					Image badgeBack = GetChildImage(((Component)(object)button).transform, "RunShopIconBadgeBack");
					if ((Object)(object)badgeBack != null)
					{
						((Graphic)badgeBack).color = Color.Lerp(modalColor, accent, 0.46f);
					}
				}
			}
		}
		...
		private static void SetIdentityLine(Image image, bool visible, Color color, Vector2 size)
		{
			...
			{
				((Component)(object)image).gameObject.SetActive(visible);
		...
		private void RefreshUi(int round)
		{
			...
			if ((Object)(object)subtitleText != null)
			{
				...
				string opportunityLabel = (currentShopIsMini ? ("고정 선택가 " + ResolveFixedMiniShopSpendTarget(round) + "G 안팎") : (currentShopIsRecovery ? "위기 지원 고정가" : ((gameController != null) ? ("현재 소환비 " + gameController.SummonCost + "G") : "소환비 확인")));
				subtitleText.text = (currentShopIsInsurance ? ("보험 추천 1개  |  " + ((gameController != null) ? gameController.BadLuckInsuranceReason : "초반 저점 복구") + "  |  " + DailyFortuneSystem.TodaySummary) : (currentShopIsRecovery ? ("3개 중 1개  |  " + opportunityLabel + "  |  " + ((gameController != null) ? gameController.EarlyRunRecoveryCause : "위기 복구")) : (currentShopIsMini ? ("ROUND " + round + "  |  " + phaseLabel + "  |  " + opportunityLabel + "  |  3개 중 1개") : ("ROUND " + round + "  |  " + opportunityLabel + "  |  유닛 소환 vs 상점 투자"))));
			}
			...
			for (int i = 0; i < buttonCount; i++)
			{
				...
				if ((Object)(object)offerButtons[i] != null)
				{
					((Component)(object)offerButtons[i]).gameObject.SetActive(show);
				}
				if (show)
				{
					ShopOffer offer = currentOffers[i];
					...
					if ((Object)(object)accent != null)
					{
						((Graphic)accent).color = offer.color;
					}
				}
			}
		}
		...
		private int ResolveOfferSummonEquivalent(ShopOffer offer)
		{
			...
			return (offer != null && offer.cost > 0) ? Mathf.Max(1, Mathf.CeilToInt((float)offer.cost / (float)summonReferenceCost)) : 0;
		}
		...
		private void Close()
		{
			...
			string passedShopLabel = (currentShopIsRecovery ? "긴급 지원" : (currentShopIsMini ? "소형 전투상점" : "전투상점"));
			currentOffers.Clear();
			currentShopIsMini = false;
			currentShopIsRecovery = false;
			currentShopIsInsurance = false;
			...
			if (passedShop)
			{
				gameController?.RequestBanner(passedShopLabel + " 패스", new Color(0.66f, 0.72f, 0.84f), 1.6f);
			}
		}
		...
		private void UpdateReopenButton()
		{
			if ((Object)(object)reopenButton != null)
			{
				...
				((Component)(object)reopenButton).gameObject.SetActive(currentOffers.Count > 0 && !panelOpen);
				Text label = ((Component)(object)reopenButton).GetComponentInChildren<Text>();
