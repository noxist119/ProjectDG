...
namespace DefenseGame
{
	public static class RuntimeKoreanTextUtility
	{
		...
		public static string Clean(string key, string value)
		{
			...
			string overrideValue = TryGetOverride(key);
			if (!string.IsNullOrEmpty(overrideValue) && LooksCorrupted(value))
			{
				return overrideValue;
			}
			string replaced = ApplyKnownReplacements(value);
			if (!LooksCorrupted(replaced))
			{
				return replaced;
			}
			string repaired = ApplyKnownReplacements(TryRepairUtf8Mojibake(replaced));
			return (SuspiciousScore(repaired) <= SuspiciousScore(replaced)) ? repaired : replaced;
		}
		...
		public static string BuildKey(Text text)
		{
			...
			Transform parent = ((Component)(object)text).transform.parent;
			return (parent == null) ? ((UnityEngine.Object)(object)text).name : (parent.name + "/" + ((UnityEngine.Object)(object)text).name);
		}
		...
		private static string TryGetOverride(string key)
		{
			...
			string localName = ((slashIndex >= 0) ? key.Substring(slashIndex + 1) : key);
			return ExactOverrides.TryGetValue(localName, out value) ? value : null;
		}

		private static string ApplyKnownReplacements(string value)
		{
			string result = value;
			foreach (KeyValuePair<string, string> replacement in KnownReplacements)
			{
