...
namespace DefenseGame
{
	public static class RuntimeAudioUtility
	{
		...
		public static void Play(string resourcePath, float volume = 1f, float minimumInterval = 0f)
		{
			...
			if (!(minimumInterval > 0f) || !LastPlayedTimes.TryGetValue(normalizedPath, out var lastPlayed) || !(now - lastPlayed < minimumInterval))
			{
		...
		{
			if (ClipCache.TryGetValue(normalizedPath, out var cached))
			{
				return cached;
			}
