...
namespace DefenseGame
{
	...
	public class MonsterCombatTuningConfig : ScriptableObject
	{
		...
		public void ApplyToMonster(MonsterDefinition definition)
		{
			...
			MonsterCombatTuningEntry entry = entries.Find((MonsterCombatTuningEntry candidate) => candidate != null && candidate.monsterId == tuningId);
			if (entry == null && TryGetOrderedEntry(tuningId, out var orderedEntry))
			{
				entry = orderedEntry;
			}
			if (entry != null)
		...
		private void ApplyDefaultSkillFx(MonsterDefinition definition)
		{
			...
			for (int i = 0; i < definition.skills.Count; i++)
			{
				...
				bool supportSkill = skill.effectType == SkillEffectType.BossFortify || skill.effectType == SkillEffectType.HealSelf || skill.effectType == SkillEffectType.MonsterRally || skill.effectType == SkillEffectType.MoveSpeedBoost || skill.effectType == SkillEffectType.AttackSpeedBoost || skill.effectType == SkillEffectType.CriticalBoost || skill.effectType == SkillEffectType.ManaSurge || skill.effectType == SkillEffectType.DamageReflect;
				bool areaSkill = skill.effectType == SkillEffectType.AreaDamage || skill.effectType == SkillEffectType.SummonRush || skill.effectType == SkillEffectType.MonsterRally;
				if (supportSkill)
				{
		...
		private void ApplySkillResourceOverrides(MonsterDefinition definition, MonsterCombatTuningEntry entry)
		{
			...
			for (int i = 1; i < definition.skills.Count; i++)
			{
				...
				if (skill != null)
				{
					...
					{
						skill.projectilePrefab = visualFallback.projectilePrefab;
					}
					if (skill.muzzleEffectPrefab == null)
					{
						skill.muzzleEffectPrefab = visualFallback.muzzleEffectPrefab;
					}
					if (skill.hitEffectPrefab == null)
					{
						skill.hitEffectPrefab = visualFallback.hitEffectPrefab;
					}
					if (skill.areaEffectPrefab == null)
					{
						skill.areaEffectPrefab = visualFallback.areaEffectPrefab;
					}
				}
			}
		}
		...
		private bool TryGetOrderedEntry(string definitionId, out MonsterCombatTuningEntry entry)
		{
			...
			List<MonsterCombatTuningEntry> ordered = entries.FindAll((MonsterCombatTuningEntry candidate) => candidate != null);
