...
namespace DefenseGame
{
	public class TacticalMissionSystem : MonoBehaviour
	{
		...
		private void WireUi()
		{
			if ((UnityEngine.Object)(object)summaryButton != null)
			{
				((UnityEvent)(object)summaryButton.onClick).RemoveListener((UnityAction)TogglePanel);
				((UnityEvent)(object)summaryButton.onClick).AddListener((UnityAction)TogglePanel);
				SetChildText(((Component)(object)summaryButton).transform, "MissionOpenHint", "보기");
			}
			if ((UnityEngine.Object)(object)closeButton != null)
			{
				((UnityEvent)(object)closeButton.onClick).RemoveListener((UnityAction)ClosePanel);
				((UnityEvent)(object)closeButton.onClick).AddListener((UnityAction)ClosePanel);
			}
			...
			{
				if (!((UnityEngine.Object)(object)optionButtons[i] == null))
				{
					((UnityEventBase)(object)optionButtons[i].onClick).RemoveAllListeners();
					((Selectable)optionButtons[i]).interactable = true;
					SetChildText(((Component)(object)optionButtons[i]).transform, "PickLabel", "진행");
				}
			}
		}
		...
		private void RefillMissions()
		{
			...
			int guard = 0;
			while (activeMissions.Count < 3 && guard < 40)
			{
		...
		private MissionInstance CreateNextMissionCandidate(int attempt)
		{
			...
			while (completedMissionKeys.Contains(kind.ToString() + ":" + tier))
			{
				tier++;
			}
			missionCursor++;
		...
		private MissionInstance CreateMission(MissionKind kind, int tier)
		{
			...
			case MissionKind.RoleCollector:
				...
				mission.summonDiscount = Mathf.Min(0.03f + (float)tier * 0.01f, 0.08f);
			...
			case MissionKind.SummonSprint:
				...
				mission.rewardText = "+" + mission.goldReward + "골드";
				mission.color = new Color(0.3f, 0.76f, 1f);
				break;
			...
			case MissionKind.LegendaryHunt:
				...
				mission.rewardText = "+" + mission.goldReward + "골드, 라운드 보너스 +" + mission.roundGoldBonus;
				mission.color = new Color(1f, 0.68f, 0.2f);
				break;
			...
			case MissionKind.BossSlayer:
				...
				mission.jackpotChance = Mathf.Min(0.18f + (float)tier * 0.025f, 0.32f);
			...
			case MissionKind.NoSummonHold:
				...
				mission.summonDiscount = Mathf.Min(0.04f + (float)tier * 0.01f, 0.1f);
			...
			case MissionKind.KillStreak:
				...
				mission.jackpotChance = Mathf.Min(0.12f + (float)tier * 0.02f, 0.26f);
			...
			case MissionKind.HighGradeForge:
				mission.target = 1;
				mission.secondaryTarget = ((tier >= 4) ? 3 : ((tier < 2) ? 1 : 2));
				mission.targetRound = round + 4;
				mission.earliestCompleteRound = round;
				mission.goldReward = 36 + tier * 12;
				mission.rouletteGoldMin = 12 + tier * 5;
				mission.rouletteGoldMax = 42 + tier * 10;
				mission.jackpotChance = Mathf.Min(0.15f + (float)tier * 0.025f, 0.34f);
			...
			case MissionKind.SpendDownGambit:
				mission.target = Mathf.Max(6, Mathf.RoundToInt((float)((gameController != null) ? gameController.SummonCost : 10) * 0.65f) + tier * 2);
				mission.targetRound = round + 1;
				mission.goldReward = 18 + tier * 7;
				mission.rouletteGoldMin = 14 + tier * 5;
				mission.rouletteGoldMax = 44 + tier * 11;
				mission.jackpotChance = Mathf.Min(0.1f + (float)tier * 0.018f, 0.24f);
				...
				mission.color = new Color(1f, 0.42f, 0.3f);
				break;
			case MissionKind.UltimateRecipeChase:
				...
				mission.jackpotChance = Mathf.Min(0.18f + (float)tier * 0.03f, 0.38f);
			...
			case MissionKind.GradeRainbow:
				...
				mission.goldReward = 30 + tier * 10;
				mission.summonDiscount = Mathf.Min(0.03f + (float)tier * 0.012f, 0.09f);
		...
		private void ApplyRewardPacing(MissionInstance mission)
		{
			if (mission != null)
			{
				...
				mission.goldReward = Mathf.Max(rewardFloor, Mathf.RoundToInt((float)mission.goldReward * goldMultiplier));
				if (mission.rouletteGoldMax > 0)
				{
					mission.rouletteGoldMin = Mathf.Max(1, Mathf.RoundToInt((float)mission.rouletteGoldMin * goldMultiplier));
					mission.rouletteGoldMax = Mathf.Max(mission.rouletteGoldMin, Mathf.RoundToInt((float)mission.rouletteGoldMax * goldMultiplier));
				}
				if (mission.jackpotGold > 0)
				{
					mission.jackpotGold = Mathf.Max(rewardFloor, Mathf.RoundToInt((float)mission.jackpotGold * goldMultiplier));
		...
		private bool IsMissionComplete(MissionInstance mission, bool roundCompleted, int completedRound)
		{
			...
			case MissionKind.HighGradeForge:
			{
				CharacterGrade requiredForgeGrade = (CharacterGrade)Mathf.Clamp(mission.secondaryTarget, 0, 5);
		...
		private void PayPendingRewards(int completedRound)
		{
			...
			if (rewardHighlights != null)
			{
				for (int j = 0; j < rewardHighlights.Count; j++)
				{
					AddCompletionFeed(rewardHighlights[j]);
				}
			}
		...
		{
			if (!((UnityEngine.Object)(object)summaryText == null))
			{
		...
		{
			if ((UnityEngine.Object)(object)panelHeaderText != null)
			{
			...
			for (int i = 0; i < buttonCount; i++)
			{
				...
				if ((UnityEngine.Object)(object)optionButtons[i] != null)
				{
					((Component)(object)optionButtons[i]).gameObject.SetActive(show);
					((Selectable)optionButtons[i]).interactable = true;
					SetChildText(((Component)(object)optionButtons[i]).transform, "PickLabel", "진행");
				}
				if (show)
				{
					...
					if ((UnityEngine.Object)(object)accent != null)
					{
						((Graphic)accent).color = mission.color;
					}
				}
			}
		}
		...
		private string GetProgressText(MissionInstance mission)
		{
			...
			case MissionKind.HighGradeForge:
			{
				CharacterGrade forgeGrade = (CharacterGrade)Mathf.Clamp(mission.secondaryTarget, 0, 5);
		...
		private int CountUnitsAtLeast(CharacterGrade grade)
		{
			...
			for (int i = 0; i < defenders.Length; i++)
			{
				if (defenders[i] != null && defenders[i].Grade >= grade)
				{
					count++;
				}
			}
		...
		private void ShowToast(string title, string reward)
		{
			...
			completionToastRoot.SetActive(value: true);
			toastTimer = 2.4f;
		...
		private void UpdateCompletionToast()
		{
			if (!(completionToastRoot == null) && completionToastRoot.activeSelf)
			{
				...
				float normalized = Mathf.Clamp01(toastTimer / 2.4f);
				float alpha = Mathf.Min(Mathf.Clamp01((2.4f - toastTimer) / 0.18f), Mathf.Clamp01(normalized / 0.2f));
				if (completionToastGroup != null)
				{
		...
		private void HideCompletionToast()
		{
			...
			if (completionToastRoot != null)
			{
				completionToastRoot.SetActive(value: false);
			}
		}
