...
public class RoundManager : MonoBehaviour
{
	...
	private class RoundSpawnTimingStep
	{
		...
		{
			float num = Mathf.Max(0, round - Mathf.Max(1, firstRound));
			float num2 = num * intervalChangePerRound;
			float num3 = ((intervalChangePerRound < 0f) ? Mathf.Max(num2, 0f - Mathf.Max(0f, maxIntervalChange)) : Mathf.Min(num2, Mathf.Max(0f, maxIntervalChange)));
			return intervalAtFirstRound + num3;
		}
	}
	...
	private void Awake()
	{
		if ((spawnPoints == null || spawnPoints.Length == 0) && ((Component)this).transform.childCount > 0)
		{
			spawnPoints = (Transform[])(object)new Transform[((Component)this).transform.childCount];
			for (int i = 0; i < ((Component)this).transform.childCount; i++)
			{
				spawnPoints[i] = ((Component)this).transform.GetChild(i);
			}
		}
	}
	...
	{
		float num = Mathf.Max(0f, accelerationStartSeconds);
		float num2 = Mathf.Max(0.1f, accelerationStepSeconds);
		if (elapsedRealtime < num)
		{
			return 1;
		}
		int num3 = 2 + Mathf.FloorToInt((elapsedRealtime - num) / num2);
		return Mathf.Clamp(num3, 2, Mathf.Max(2, maxMultiplier));
	}
	...
	private void UpdateCombatTimeAcceleration()
	{
		...
		{
			combatTimeLastRealtime = unscaledTime;
			return;
		}
		CombatElapsedRealtime += Mathf.Max(0f, unscaledTime - combatTimeLastRealtime);
		combatTimeLastRealtime = unscaledTime;
		int num = ResolveCombatTimeScaleMultiplier(CombatElapsedRealtime, longCombatAccelerationStartSeconds, longCombatAccelerationStepSeconds, longCombatAccelerationMaxMultiplier);
		if (num != CombatTimeScaleMultiplier && Mathf.Approximately(Time.timeScale, combatSpeedAppliedTimeScale))
		{
			CombatTimeScaleMultiplier = num;
	...
	private static bool IsCombatTimeAccelerationPaused()
	{
		...
		DefenseGameController active = DefenseGameController.Active;
		return ((Object)(object)active != (Object)null && active.FateChoiceSlowMotionActive) || Time.timeScale < 0.5f;
	}
	...
	public void StartNextRound()
	{
		if (!IsRoundRunning)
		{
			roundRoutine = ((MonoBehaviour)this).StartCoroutine(RunRound());
		}
	}

	public int GetNextBossRound(int fromRound)
	{
		int num = Mathf.Max(0, fromRound);
		int num2 = Mathf.Max(10, (num / 10 + 1) * 10);
		return (num2 <= num) ? (num + 10) : num2;
	}
	...
	public void BeginDefeatCinematic()
	{
		if (IsRoundRunning)
		{
			...
			{
				((MonoBehaviour)this).StopCoroutine(roundRoutine);
				roundRoutine = null;
			}
	...
	private IEnumerator RunRound()
	{
		...
		for (int regularIndex = 0; regularIndex < spawnPlan.regularMonsterCount; regularIndex++)
		{
			...
			yield return (object)new WaitForSeconds(GetSpawnDelay(spawnPlan, regularIndex));
		}
		...
		{
			yield return (object)new WaitForSeconds(Mathf.Max(0f, bossEntryDelay));
	...
	{
		yield return (object)new WaitForSeconds(Mathf.Max(minimumSpawnInterval, midBossEntryDelay));
		...
		yield return (object)new WaitForSeconds(Mathf.Max(minimumSpawnInterval, spawnPlan.interval * 0.85f));
	}
	...
	{
		bool flag = round > 0 && round % 10 == 0;
		int num = CalculateMidBossCount(round, flag);
		if (num > 0 && ((Object)(object)monsterDatabase == (Object)null || !monsterDatabase.HasMidBossForRound(round)))
		{
			num = 0;
		}
		int num2 = (flag ? CalculateBossSupportCount(round) : Mathf.Max(3, CalculateRegularMonsterCount(round) - num * 2));
		if (!flag)
		{
			float num3 = (((Object)(object)DefenseGameController.Active != (Object)null) ? DefenseGameController.Active.GetFateMonsterCountMultiplierForRound(round) : 1f);
			if (num3 > 1f)
			{
				num2 = Mathf.CeilToInt((float)num2 * num3);
			}
		}
		num2 = ApplyPreBossLeakEaseToRegularCount(round, flag, num2);
		num2 = CommercialRoundPacing.ApplySpawnCount(round, flag, num2);
		float interval = CommercialRoundPacing.ApplySpawnInterval(round, flag, CalculateSpawnInterval(round, flag));
		return new RoundSpawnPlan
		{
			regularMonsterCount = Mathf.Max(0, num2),
			midBossMonsterCount = Mathf.Max(0, num),
			bossMonsterCount = (flag ? 1 : 0),
			interval = interval,
	...
	private int CalculateRegularMonsterCount(int round)
	{
		...
		if (TryResolveSpawnBalanceStep(round, out var step))
		{
			float num = step.regularCountAtFirstRound + (float)(round - step.firstRound) * step.regularCountPerRound;
			int num2 = Mathf.RoundToInt(num);
			if (IsPressureRound(round, step))
			{
				num2 += Mathf.Max(0, step.pressureBonus);
			}
			int num3 = ((step.maxRegularCount > 0) ? step.maxRegularCount : maxRegularMonsterCount);
			return Mathf.Clamp(num2, 1, Mathf.Max(1, num3));
		}
		int num4 = ((round <= 3) ? (earlyRoundMonsterCount + (round - 1) * Mathf.Max(0, earlyRoundMonsterStep)) : ((round > 9) ? Mathf.RoundToInt((float)(midRoundMonsterCount + 5 * Mathf.Max(0, midRoundMonsterStep)) + (float)(round - 9) * Mathf.Max(0f, lateRoundMonsterStep)) : (midRoundMonsterCount + (round - 4) * Mathf.Max(0, midRoundMonsterStep))));
		if (IsPressureRound(round))
		{
			num4 += Mathf.Max(0, pressureRoundBonus);
		}
		return Mathf.Clamp(num4, 1, Mathf.Max(1, maxRegularMonsterCount));
	}
	...
	private int CalculateMidBossCount(int round, bool bossRound)
	{
		if (midBossCadenceSteps != null && midBossCadenceSteps.Count > 0)
		{
			...
			{
				MidBossCadenceStep midBossCadenceStep = midBossCadenceSteps[i];
				if (midBossCadenceStep != null && midBossCadenceStep.AppliesTo(round, bossRound, IsPressureRound(round)))
				{
					num = Mathf.Max(num, midBossCadenceStep.count);
				}
			}
			return Mathf.Clamp(num, 0, Mathf.Max(0, maxMidBossesPerRound));
		}
		...
		if (bossRound)
		{
			int num2 = bossRoundMidBossCount;
			if (round >= 30)
			{
				num2++;
			}
			return Mathf.Clamp(num2, 0, Mathf.Max(0, maxMidBossesPerRound));
		}
		int num3 = 0;
		...
		return Mathf.Clamp(num3, 0, Mathf.Max(0, maxMidBossesPerRound));
	}
	...
	private int CalculateBossSupportCount(int round)
	{
		...
		{
			int num = Mathf.Max(0, (round - Mathf.Max(1, bossSupportBalance.firstBossRound)) / Mathf.Max(1, bossSupportBalance.bossRoundFrequency));
			int num2 = bossSupportBalance.baseSupportCount + num * Mathf.Max(0, bossSupportBalance.supportCountPerBossEncounter);
			int num3 = ((bossSupportBalance.maxSupportCount > 0) ? bossSupportBalance.maxSupportCount : maxRegularMonsterCount);
			return Mathf.Clamp(num2, Mathf.Max(0, bossSupportBalance.minimumSupportCount), Mathf.Max(3, num3));
		}
		int num4 = bossSupportMonsterCount + Mathf.Max(0, (round - 10) / 10) * 2;
		return Mathf.Clamp(num4, 3, Mathf.Max(3, maxRegularMonsterCount));
	}
	...
	private float CalculateSpawnInterval(int round, bool bossRound)
	{
		...
		if (TryResolveSpawnTimingStep(round, timingSteps, out var step))
		{
			float num = step.ResolveInterval(round);
			if (!bossRound && IsPressureRound(round))
			{
				num -= Mathf.Max(0f, step.pressureIntervalPenalty);
			}
			return Mathf.Max(minimumSpawnInterval, ApplyPreBossLeakEaseToSpawnInterval(round, bossRound, num));
		}
		float num2 = (bossRound ? (bossSupportSpawnInterval - Mathf.Min(0.18f, (float)round * 0.004f)) : ((round <= 3) ? (earlyRoundSpawnInterval - (float)(round - 1) * 0.04f) : ((round > 9) ? (lateRoundSpawnInterval - Mathf.Min(0.14f, (float)(round - 10) * 0.008f)) : (midRoundSpawnInterval - (float)(round - 4) * 0.025f))));
		if (IsPressureRound(round) && !bossRound)
		{
			num2 -= 0.06f;
		}
		return Mathf.Max(minimumSpawnInterval, ApplyPreBossLeakEaseToSpawnInterval(round, bossRound, num2));
	}

	private int[] BuildMidBossSpawnIndices(int regularCount, int midBossCount)
	{
		int[] array = new int[Mathf.Max(0, midBossCount)];
		if (array.Length == 0)
		{
			return array;
		}
		int num = Mathf.Max(0, regularCount);
		for (int i = 0; i < array.Length; i++)
		{
			float num2 = ((array.Length == 1) ? midBossEntryRatio : Mathf.Lerp(0.38f, 0.72f, (array.Length <= 1) ? 0f : ((float)i / (float)(array.Length - 1))));
			array[i] = Mathf.Clamp(Mathf.RoundToInt((float)num * num2), 0, num);
		}
		return array;
	}
	...
	{
		float num = Mathf.Sin(((float)CurrentRound * 0.77f + (float)spawnIndex * 1.31f) * MathF.PI);
		float num2 = plan.interval + num * plan.intervalVariance;
		return Mathf.Max(minimumSpawnInterval, num2);
	}
	...
	private IEnumerator WaitForSpawnBudget(MonsterThreatLevel threatLevel)
	{
		...
		{
			yield return (object)new WaitForSeconds(Mathf.Max(0.02f, spawnBudgetPollInterval));
		}
	}
	...
	private bool IsPressureRound(int round, RoundSpawnBalanceStep balanceStep)
	{
		...
		int num = ((balanceStep.pressureRoundFrequency > 0) ? balanceStep.pressureRoundFrequency : pressureRoundFrequency);
		return num > 0 && round > 0 && round % num == 0 && round % 10 != 0;
	}
	...
	private bool TryResolveSpawnBalanceStep(int round, out RoundSpawnBalanceStep step)
	{
		...
		{
			RoundSpawnBalanceStep roundSpawnBalanceStep = regularSpawnBalanceSteps[i];
			if (roundSpawnBalanceStep != null && roundSpawnBalanceStep.firstRound <= round && roundSpawnBalanceStep.firstRound >= num)
			{
				num = roundSpawnBalanceStep.firstRound;
				step = roundSpawnBalanceStep;
			}
		}
		return step != null;
	}
	...
	private bool TryResolveSpawnTimingStep(int round, List<RoundSpawnTimingStep> timingSteps, out RoundSpawnTimingStep step)
	{
		...
		{
			RoundSpawnTimingStep roundSpawnTimingStep = timingSteps[i];
			if (roundSpawnTimingStep != null && roundSpawnTimingStep.firstRound <= round && roundSpawnTimingStep.firstRound >= num)
			{
				num = roundSpawnTimingStep.firstRound;
				step = roundSpawnTimingStep;
			}
		}
		return step != null;
	}
	...
	private bool SpawnMonster(MonsterDefinition definition)
	{
		...
		if (definition == null || spawnPoints == null || spawnPoints.Length == 0 || (Object)(object)goalPoint == (Object)null)
		{
			Debug.LogWarning((object)"RoundManager references are missing.");
			return false;
		}
		GameObject val = (((Object)(object)definition.prefab != (Object)null) ? definition.prefab : (((Object)(object)fallbackMonsterPrefab != (Object)null) ? ((Component)fallbackMonsterPrefab).gameObject : null));
		if ((Object)(object)val == (Object)null)
		{
			Debug.LogError((object)"No MonsterUnit prefab assigned.");
			return false;
		}
		Transform nextSpawnPoint = GetNextSpawnPoint();
		Quaternion val2 = (((Object)(object)nextSpawnPoint != (Object)null) ? nextSpawnPoint.rotation : Quaternion.identity);
		if ((Object)(object)nextSpawnPoint != (Object)null && (Object)(object)goalPoint != (Object)null)
		{
			val2 = RuntimeEffectUtility.FaceTowards(nextSpawnPoint.position, goalPoint.position, val2);
		}
		PlaySpawnEffect(nextSpawnPoint, val2);
		GameObject val3 = Object.Instantiate<GameObject>(val, nextSpawnPoint.position, val2);
		MonsterUnit monsterUnit = val3.GetComponent<MonsterUnit>();
		if ((Object)(object)monsterUnit == (Object)null)
		{
			monsterUnit = val3.AddComponent<MonsterUnit>();
		}
		if ((Object)(object)fallbackMonsterPrefab != (Object)null)
		{
			monsterUnit.AdoptRuntimeTemplate(fallbackMonsterPrefab);
		}
		((Component)monsterUnit).gameObject.SetActive(true);
		monsterUnit.Initialize(definition, goalPoint, CurrentRound);
		return true;
	}
	...
	private void PlaySpawnEffect(Transform spawnPoint, Quaternion spawnRotation)
	{
		...
		if (!((Object)(object)spawnPoint == (Object)null) && !((Object)(object)spawnEffectPrefab == (Object)null))
		{
	...
	private Transform GetNextSpawnPoint()
	{
		...
		Transform result = spawnPoints[nextSpawnPointIndex % spawnPoints.Length];
		nextSpawnPointIndex++;
		return result;
	}
}
