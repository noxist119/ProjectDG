...
public static class SkillDefinitionUtility
{
	...
	{
		string text = FormatPercent(skill.power);
		string text2 = FormatPercent((skill.secondaryPower > 0f) ? skill.secondaryPower : skill.power);
		if (text == text2)
		{
			return "현재 체력과 공격력의 " + text + " 수준인 미니미를 소환해 적과 싸우게 합니다.";
		}
		return "현재 체력의 " + text + ", 공격력의 " + text2 + " 수준인 미니미를 소환해 적과 싸우게 합니다.";
	}
	...
	private static void AppendGrowthTarget(ref string result, SkillDefinition skill, SkillGrowthTarget target)
	{
		...
		string growthTargetLabel = GetGrowthTargetLabel(skill, target);
		if (!string.IsNullOrEmpty(growthTargetLabel))
		{
			if (!string.IsNullOrEmpty(result))
			{
				result += ", ";
			}
			result += growthTargetLabel;
		}
	}
