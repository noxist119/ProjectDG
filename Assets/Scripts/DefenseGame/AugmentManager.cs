...
namespace DefenseGame
{
    public enum AugmentEffectType
    {
        ...
        SkillPowerMultiplier = 7,
        CriticalDamageMultiplier = 8,
        BossDamageMultiplier = 9,
        RoundStartBurst = 10,
        SummonCostDiscount = 11,
        RoundGoldBonus = 12,
        InstantGold = 13,
        WorkerGoldPerRound = 14,
        RandomInstantGold = 15,
        KillGoldChance = 16,
        RoundClearRandomGold = 17,
        BossRoundBet = 18,
        InterestGold = 19,
        ScalingAttackPowerPerRound = 20,
        ScalingAttackSpeedPerRound = 21,
        ScalingSkillPowerPerRound = 22,
        ScalingManaRegenPerRound = 23,
        ScalingBossDamagePerRound = 24,
        ScalingGoldPerRound = 25,
        Hero08PetrifyBloom = 26,
        Hero01VolcanicAftershock = 27,
        Hero03FrostResidue = 28,
    ...
    public class AugmentManager : MonoBehaviour
    {
        ...
        public bool TryGrantShopAugment(string id)
        {
            ...
            AugmentDefinition selected = augmentPool.Find(augment =>
                augment != null && string.Equals(augment.id, id, StringComparison.Ordinal));
            if (selected == null)
            {
                return false;
            }
        ...
        public void OpenPendingChoice()
        {
            ...
            SetChoiceOpen(true);
            OnChoiceShown?.Invoke(pendingChoiceRound);
        }
        ...
        private void TickHoardInterestGold()
        {
            ...
            Color rewardColor = new Color(1f, 0.90f, 0.30f);
            ...
            hoardIncomeTimer -= tickInterval * ticks;
        ...
        private void BindButtons()
        {
            if (closeButton != null)
            {
                closeButton.onClick.RemoveListener(CloseChoice);
                closeButton.onClick.AddListener(CloseChoice);
            }

            if (reopenButton != null)
            {
                reopenButton.onClick.RemoveListener(OpenPendingChoice);
                reopenButton.onClick.AddListener(OpenPendingChoice);
            }
            ...
            for (int i = 0; i < choiceButtons.Length; i++)
            {
                ...
                choiceButtons[i].onClick.AddListener(() => ChooseAugment(choiceIndex));
            }
        }
        ...
        private void HandleRoundStarted(int round)
        {
            ...
            for (int i = 0; i < chosenAugments.Count; i++)
            {
                ...
                if (IsBuildupEffect(augment.effectType))
                {
                    ...
                    for (int defenderIndex = 0; defenderIndex < defenders.Length; defenderIndex++)
                    {
                        ApplyBuildupIncrement(defenders[defenderIndex], augment);
                    }
                }
        ...
        private void HandleShieldResolved(DefenderUnit shielded, float blockedDamage, bool shieldBroken, MonsterUnit source)
        {
            ...
            {
                ApplyHeroAreaDamage(shielded, null, shielded.transform.position, 2.6f, Mathf.Max(5f, blockedDamage * 0.80f));
            }

            if (source != null && HasChosen("hero05_reflect_r"))
            {
                source.TakeDamage(Mathf.Max(1f, blockedDamage * 0.45f), false, shielded);
            }
        ...
        {
            if (defender == null || damage <= 0f)
            {
                return;
            }

            if (hero54StoredDamage.ContainsKey(defender))
            {
        ...
        private void ResolveHeroMonsterKilledAugments(MonsterUnit monster)
        {
            ...
            if (string.Equals(killerId, "hero_01", StringComparison.OrdinalIgnoreCase) &&
                HasChosen("hero01_mana_refund_r") &&
                monster.LastDamageSkill != null)
            {
                killer.RestoreMana(0.50f);
            }

            if (string.Equals(killerId, "hero_04", StringComparison.OrdinalIgnoreCase) &&
                HasChosen("hero04_contagion_r") &&
                !monster.IsBoss)
            {
            ...
            if (string.Equals(killerId, "hero_52", StringComparison.OrdinalIgnoreCase) &&
                (HasChosen("hero52_meteor_extend_r") || HasChosen("hero52_star_shower_m")) &&
                monster.LastDamageSkill != null)
            {
                int zoneCount = HasChosen("hero52_star_shower_m") ? 3 : 1;
            ...
            if (gameController != null &&
                string.Equals(killerId, "hero_08", StringComparison.OrdinalIgnoreCase) &&
                HasChosen("hero08_trophy_m") &&
                !monster.IsBoss &&
                UnityEngine.Random.value <= 0.30f)
            {
                gameController.AddGold(2);
                ShowEconomyBanner("전리품 석상 +2G", new Color(0.74f, 0.70f, 1f), 1.2f, 0.8f);
            }
        }
        ...
        private void ShowChoices(int round)
        {
            ...
            LoadLastRunChoiceIds(round);
            AugmentStyle[] slots = BuildChoiceStyleSlots(round);
            for (int i = 0; i < slots.Length; i++)
            {
                AugmentDefinition choice = PickChoice(slots[i], RollChoiceRarity(round));
            ...
            for (int i = 0; i < choiceButtons.Length; i++)
            {
                ...
                {
                    accentImages[i].color = choice.accentColor;
                }

                if (styleTexts != null && i < styleTexts.Length && styleTexts[i] != null)
                {
                    styleTexts[i].text = GetChoiceRarityLabel(choice) + "  |  " + GetChoiceLabel(choice);
                    styleTexts[i].color = Color.white;
                }

                if (titleTexts != null && i < titleTexts.Length && titleTexts[i] != null)
                {
                    titleTexts[i].text = choice.title;
                    titleTexts[i].color = choice.accentColor;
                }

                Outline outline = choiceButtons[i].GetComponent<Outline>();
                if (outline != null)
                {
                    outline.effectColor = GetChoiceRarityColor(choice);
                }
        ...
        private static ChoiceRarity GetChoiceRarity(AugmentDefinition augment)
        {
            if (augment == null) return ChoiceRarity.Normal;
            if (augment.heroTier == HeroAugmentTier.Mythic) return ChoiceRarity.Legendary;
            if (augment.heroTier == HeroAugmentTier.Rare) return ChoiceRarity.Rare;
            return augment.rarity;
        }

        private static string GetChoiceRarityLabel(AugmentDefinition augment)
        {
            ChoiceRarity rarity = GetChoiceRarity(augment);
            return rarity == ChoiceRarity.Legendary ? "\uc804\uc124" : rarity == ChoiceRarity.Rare ? "\ub808\uc5b4" : "\uc77c\ubc18";
        }

        private static Color GetChoiceRarityColor(AugmentDefinition augment)
        {
            ChoiceRarity rarity = GetChoiceRarity(augment);
            return rarity == ChoiceRarity.Legendary ? new Color(1f, 0.72f, 0.20f, 1f) :
                rarity == ChoiceRarity.Rare ? new Color(0.24f, 0.62f, 1f, 1f) : new Color(0.68f, 0.70f, 0.78f, 1f);
        }
        ...
        private bool HasChoiceFamily(AugmentDefinition augment)
        {
            ...
            for (int i = 0; i < currentChoices.Count; i++)
            {
                if (GetAugmentOfferFamily(currentChoices[i]) == family)
                {
                    return true;
                }
            }
        ...
        private bool WasRecentlyOfferedFamily(AugmentDefinition augment)
        {
            string family = GetAugmentOfferFamily(augment);
            return !string.IsNullOrEmpty(family) && recentAugmentOfferFamilies.Contains(family);
        }

        private static string GetAugmentOfferFamily(AugmentDefinition augment)
        {
            if (augment == null)
            {
                return string.Empty;
            ...
            {
                return "hero:" + augment.requiredHeroId;
            }
            ...
            switch (augment.effectType)
            {
                ...
                case AugmentEffectType.SkillManaRelay:
                case AugmentEffectType.SkillChainBlast:
                    return "skill";
                ...
                case AugmentEffectType.KillGoldChance:
                case AugmentEffectType.RoundClearRandomGold:
                case AugmentEffectType.InterestGold:
                case AugmentEffectType.HoardInterestGold:
                    return "economy";
                default:
                    return augment.effectType.ToString();
            }
        }
        ...
        private void RememberCurrentChoices()
        {
            for (int i = 0; i < currentChoices.Count; i++)
            {
                ...
                string family = GetAugmentOfferFamily(choice);
                if (!string.IsNullOrEmpty(family))
                {
                    recentAugmentOfferFamilies.Remove(family);
                    recentAugmentOfferFamilies.Add(family);
                }
            }
            ...
            while (recentAugmentOfferFamilies.Count > historyLimit)
            {
                recentAugmentOfferFamilies.RemoveAt(0);
            }
        }
        ...
        private void UpdateReopenButton()
        {
            if (reopenButton != null)
            {
                reopenButton.gameObject.SetActive(HasPendingChoice && !IsChoiceOpen);
            }
        }
        ...
        private void ApplyBuildupBonus(DefenderUnit defender, AugmentDefinition augment, int stacks)
        {
            ...
            float amount = augment.value * stacks;
            switch (augment.effectType)
            {
        ...
        private void ResolveRoundStartedEconomy(AugmentDefinition augment, int round)
        {
            ...
            {
                int reward = Mathf.Max(1, Mathf.RoundToInt(augment.value + round * augment.secondaryValue));
                gameController.AddGold(reward);
            ...
            {
                int stacks = GetBuildupStacks(augment);
                int reward = Mathf.Max(1, Mathf.RoundToInt(augment.value * stacks));
                gameController.AddGold(reward);
                ShowEconomyBanner("복리 광산 +" + reward + "G", augment.accentColor, 1.7f, 1.2f);
            }
        }
        ...
        private void ResolveRoundCompletedEconomy(int round)
        {
            ...
            for (int i = 0; i < chosenAugments.Count; i++)
            {
                ...
                {
                    int minGold = Mathf.Max(1, Mathf.RoundToInt(augment.value));
                    int maxGold = Mathf.Max(minGold, Mathf.RoundToInt(augment.value + augment.secondaryValue));
                    int reward = Mathf.Clamp(
                        Mathf.RoundToInt(augment.value + UnityEngine.Random.Range(0f, augment.secondaryValue)),
                        minGold,
                        maxGold);
                    gameController.AddGold(reward);
                    ShowRandomGoldFeedback(augment, reward, minGold, maxGold);
                ...
                else if (augment.effectType == AugmentEffectType.InterestGold)
                {
                    ...
                    int reward = Mathf.Clamp(Mathf.RoundToInt(gameController.Gold * Mathf.Max(0f, augment.value)), 1, cap);
                    gameController.AddGold(reward);
                    ShowEconomyBanner("이자 수익 +" + reward + "G", augment.accentColor, 1.7f, 1.2f);
                }
            }
        }
        ...
        {
            return effectType == AugmentEffectType.ScalingAttackPowerPerRound ||
                effectType == AugmentEffectType.ScalingAttackSpeedPerRound ||
                effectType == AugmentEffectType.ScalingSkillPowerPerRound ||
                effectType == AugmentEffectType.ScalingManaRegenPerRound ||
                effectType == AugmentEffectType.ScalingBossDamagePerRound ||
                effectType == AugmentEffectType.ScalingGoldPerRound;
        }
        ...
        private int GetBuildupStacks(AugmentDefinition augment)
        {
            ...
            return buildupStacks.TryGetValue(augment.id, out int stacks) ? Mathf.Max(0, stacks) : 0;
        }
        ...
        private bool CanOfferAugment(AugmentDefinition augment)
        {
            ...
            {
                case AugmentEffectType.Hero08PetrifyBloom:
                    return CanOfferHeroAugment(augment, "hero_08", 0.52f);
                case AugmentEffectType.Hero01VolcanicAftershock:
                    return CanOfferHeroAugment(augment, "hero_01", 0.42f);
                case AugmentEffectType.Hero03FrostResidue:
                    return CanOfferHeroAugment(augment, "hero_03", 0.42f);
                case AugmentEffectType.Hero05GuardianProtocol:
                    return CanOfferHeroAugment(augment, "hero_05", 0.42f);
                case AugmentEffectType.Hero13ManaNetwork:
                    return CanOfferHeroAugment(augment, "hero_13", 0.42f);
                default:
                    return true;
            }
        }
        ...
        {
            if (tier == HeroAugmentTier.Mythic) return Mathf.Clamp01(mythicHeroAugmentOfferChance);
            if (tier == HeroAugmentTier.Rare) return Mathf.Clamp01(rareHeroAugmentOfferChance);
            return Mathf.Clamp01(normalHeroAugmentOfferChance);
        }
        ...
        private bool IsHero(DefenderUnit defender, string heroId)
        {
            return defender != null &&
                ...
                string.Equals(defender.Definition.id, heroId, StringComparison.OrdinalIgnoreCase);
        }
        ...
        private void TryResolveHero08PetrifyBloom(DefenderUnit source, MonsterUnit target, AugmentDefinition augment)
        {
            ...
            MonsterUnit.ApplyPetrifyRadius(
                target.transform.position,
                Mathf.Max(0.1f, augment.secondaryValue),
                new MonsterUnit.PetrifyTargetOptions
                {
        ...
        private void TryResolveHero01VolcanicAftershock(DefenderUnit source, MonsterUnit target, float damage, AugmentDefinition augment)
        {
            ...
            ApplyHeroAreaDamage(
                ...
                target,
                target.transform.position,
                Mathf.Max(0.1f, augment.duration),
                damage * Mathf.Max(0f, augment.secondaryValue));
        }
        ...
        private void ApplyHeroAreaDamage(DefenderUnit source, MonsterUnit centerTarget, Vector3 center, float radius, float damage)
        {
            ...
            try
            {
                for (int i = 0; i < monsters.Count; i++)
                {
                    ...
                    {
                        monster.TakeDamage(damage, false, source);
                    }
                }
            }
            finally
        ...
        private void ApplyHero05GuardianProtocol(DefenderUnit[] defenders, AugmentDefinition augment)
        {
            ...
            for (int i = 0; i < defenders.Length; i++)
            {
                DefenderUnit source = defenders[i];
                ...
                {
                    DefenderUnit target = defenders[j];
        ...
        {
            List<DefenderUnit> allies = FindLowestHealthDefenders(HasChosen("hero02_emergency_m") ? 3 : 2, true);
            if (HasChosen("hero02_battle_rx_n"))
            {
                ApplyCombatBoostToDefenders(allies, 0.10f, 0.08f, 4f);
            }
        ...
        private void ResolveHero04SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            if (HasChosen("hero04_toxic_pool_n"))
            {
                ...
                {
                    target.TakeDamage(attackPower * 0.40f, false, source);
                }
            }

            if (HasChosen("hero04_plague_cloud_m"))
            {
                ApplyHeroPoisonRadius(source, center, 3.1f, attackPower * 0.36f, 5f, 0.75f, attackPower * 0.90f);
        ...
        private void ResolveHero06SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            {
                int hitCount = ApplyHeroLineDamage(source, target, 5.6f, 0.70f, attackPower * 0.58f);
                if (hitCount >= 2)
                {
        ...
        private void ResolveHero07SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            if (target.IsBoss)
            {
                target.TakeDamage(attackPower * 0.45f, false, source);
            }
            else
            {
                target.ApplySlow(0.24f, 2.5f);
                target.TakeDamage(Mathf.Min(target.MaxHealth * 0.10f, attackPower * 1.15f), false, source);
            }
        }
        ...
        private void ResolveHero03SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            if (target.IsBoss)
            {
                ...
                {
                    target.TakeDamage(attackPower * 0.25f, false, source);
                }
            ...
            if (HasChosen("hero03_permafrost_r") && UnityEngine.Random.value <= 0.20f)
            {
        ...
        private void ResolveHero08SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            {
                ApplyHeroAreaDamage(source, null, target.transform.position, 3f, attackPower * 1.10f);
            }
        ...
        private void ResolveHero09SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            {
                ApplyHeroLineDamage(source, target, 6.5f, 1.05f, attackPower * 1.10f);
        ...
        private void ResolveHero10SkillAugments(DefenderUnit source, SkillDefinition skill)
        {
            ...
            {
                TriggerHeroSkillEchoes("hero_10", source, skill, 0.80f, 99);
            }
        ...
        private void ResolveHero11SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            {
                AddTrackedMaxHealth(source, hero11MaxHealthGrowth, 0.05f, 0.30f);
        ...
        private void ResolveHero12SkillAugments(DefenderUnit source, SkillDefinition skill)
        {
            ...
            if (HasChosen("hero12_double_cast_r") && UnityEngine.Random.value <= 0.35f)
            {
                TriggerSingleSkillEcho(source, skill, 0.62f);
                source.RestoreMana(0.12f);
            }
            else if (HasChosen("hero12_echo_cast_n") && UnityEngine.Random.value <= 0.20f)
            {
        ...
        private void ResolveHero13SkillAugments(DefenderUnit source)
        {
            ...
            if (HasChosen("hero13_mana_gamble_r") && UnityEngine.Random.value <= 0.50f)
            {
                source.RestoreMana(0.50f);
            }
        ...
        private void ResolveHero14SkillAugments(DefenderUnit source)
        {
            ...
            {
                List<DefenderUnit> allDefenders = FindRandomDefenders(99, true);
                ApplyCombatBoostToDefenders(allDefenders, 0.16f, 0.38f, 6f);
                RestoreLowestManaDefenders(source.transform.position, 99f, 0.10f, 3);
                return;
            }

            if (HasChosen("hero14_remote_order_r"))
            {
                ApplyCombatBoostToDefenders(FindRandomDefenders(2, true), 0.12f, 0.30f, 5f);
            }
            else if (HasChosen("hero14_remote_buff_n"))
            {
                ApplyCombatBoostToDefenders(FindRandomDefenders(1, true), 0.08f, 0.22f, 5f);
            }
        }
        ...
        private void ResolveHero22SkillAugments(DefenderUnit source, SkillDefinition skill)
        {
            ...
            {
                StartHero54RetaliationWindow(source, 4.5f, 0.35f, 3.0f, 0f);
                ShieldNearbyDefenders(source.transform.position, 3.0f, 0.08f, 4f);
            }
        ...
        private void ResolveHero23SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            {
                target.TakeDamage(attackPower * 0.75f, false, source);
            }

            if (HasChosen("hero23_weakpoint_r"))
            {
                float damage = target.CurrentHealth <= target.MaxHealth * 0.50f ? attackPower * 1.35f : attackPower * 0.55f;
                target.TakeDamage(damage, target.CurrentHealth <= target.MaxHealth * 0.50f, source);
            }

            if (HasChosen("hero23_royal_break_m"))
            {
                target.TakeDamage(attackPower * 1.30f, true, source);
        ...
        private void ResolveHero31SkillAugments(DefenderUnit source)
        {
            ...
            {
                source.ActivateTimedDamageReduction(0.30f, 5f);
                source.AddShield(source.MaxHealth * 0.10f, 5f);
            }
        ...
        private void ResolveHero32SkillAugments(DefenderUnit source, MonsterUnit target)
        {
            ...
            {
                DamageRandomMonsters(source, 2, GetHeroAttackPower(source) * 0.60f);
            }

            if (HasChosen("hero32_alpha_hunt_m"))
            {
                source.ActivateTimedCombatBoost(0.16f, 0.28f, 6f);
                DamageRandomMonsters(source, 4, GetHeroAttackPower(source) * 0.70f);
            }
        }
        ...
        private void ResolveHero51SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            else if (HasChosen("hero51_residual_lightning_r") && UnityEngine.Random.value <= 0.30f)
            {
                TriggerSingleSkillEcho(source, skill, 0.45f);
            }
        }
        ...
        private void ResolveHero54SkillAugments(DefenderUnit source, SkillDefinition skill)
        {
            ...
            {
                StartHero54RetaliationWindow(source, 5f, 0.50f, 3.4f, 0.20f);
                source.ActivateTimedDamageReduction(0.42f, 5f);
            }
            else if (HasChosen("hero54_retribution_r"))
            {
                StartHero54RetaliationWindow(source, 4f, 0.50f, 3f, 0f);
        ...
        private void ResolveHero55SkillAugments(DefenderUnit source, MonsterUnit target)
        {
            ...
            {
                ApplyHeroAreaDamage(source, target, center, 3f, attackPower * 0.90f);
            }

            if (HasChosen("hero55_mobile_fortress_m"))
            {
                ApplyHeroAreaDamage(source, target, center, 3.4f, attackPower * 1.40f);
                source.AddShield(source.MaxHealth * 0.20f, 6f);
                source.ActivateTimedDamageReduction(0.30f, 6f);
            }
        }
        ...
        private void ResolveHero56SkillAugments(DefenderUnit source, SkillDefinition skill, MonsterUnit target)
        {
            ...
            {
                TriggerSingleSkillEcho(source, skill, 0.50f);
            }

            if (HasChosen("hero56_orbital_restrike_m"))
            {
                TriggerSingleSkillEcho(source, skill, 0.80f);
                ApplyHeroAreaDamage(source, target, center, 3.6f, attackPower * 1.20f);
            }
        }
        ...
        private void ResolveHero57SkillAugments(DefenderUnit source)
        {
            ...
            {
                DamageRandomMonsters(source, 2, attackPower * 0.70f);
            }

            if (HasChosen("hero57_ricochet_r"))
            {
                DamageRandomMonsters(source, 4, attackPower * 0.80f);
            }

            if (HasChosen("hero57_chaos_mag_m"))
            {
                DamageRandomMonsters(source, 6, attackPower * 0.90f);
            }
        }
        ...
        private void RestoreLowestManaDefenders(Vector3 center, float radius, float manaRatio, int count, DefenderUnit excludedDefender = null)
        {
            ...
            candidates.Sort((a, b) => (a.CurrentMana / Mathf.Max(1f, a.MaxMana)).CompareTo(b.CurrentMana / Mathf.Max(1f, b.MaxMana)));
            int checkedCount = Mathf.Min(Mathf.Max(1, count), candidates.Count);
            for (int i = 0; i < checkedCount; i++)
            {
                candidates[i].RestoreMana(manaRatio);
            }
        }
        ...
        private List<DefenderUnit> FindLowestHealthDefenders(int count, bool includeHealthy)
        {
            ...
            for (int i = 0; i < defenders.Length; i++)
            {
                ...
                if (!includeHealthy && defender.HealthRatio >= 0.98f)
                {
                    continue;
                }
            ...
            candidates.Sort((a, b) => a.HealthRatio.CompareTo(b.HealthRatio));
            int checkedCount = Mathf.Min(Mathf.Max(1, count), candidates.Count);
        ...
        private void AddTrackedMaxHealth(DefenderUnit source, Dictionary<DefenderUnit, float> tracker, float ratio, float cap)
        {
            ...
            float current = tracker.TryGetValue(source, out float value) ? value : 0f;
        ...
        private void ApplyHeroPoisonRadius(DefenderUnit source, Vector3 center, float radius, float damagePerTick, float duration, float tickInterval, float bossDamage)
        {
            ...
            for (int i = 0; i < monsters.Count; i++)
            {
                ...
                if (monster.IsBoss)
                {
                    monster.TakeDamage(bossDamage, false, source);
                }
                else
                {
        ...
        private void ResolveHero07Kill(DefenderUnit killer)
        {
            ...
            int kills = hero07KillCounts.TryGetValue(killer, out int value) ? value + 1 : 1;
            ...
            if ((HasChosen("hero07_reaper_form_r") || HasChosen("hero07_reaper_execute_m")) &&
                kills >= 10 &&
                !hero07ReaperActive.Contains(killer))
            {
        ...
        private void ResolveHero07ReaperStrike(DefenderUnit source, MonsterUnit target)
        {
            ...
            int strikes = hero07StrikeCounts.TryGetValue(source, out int value) ? value + 1 : 1;
            ...
            try
            {
                if (target.IsBoss)
                {
                    target.TakeDamage(attackPower * 2.0f, true, source);
                }
                else
                {
                    float executeDamage = target.CurrentHealth <= target.MaxHealth * 0.45f
                        ? target.MaxHealth * 2.5f
                        : attackPower * 1.35f;
                    target.TakeDamage(executeDamage, true, source);
                }
            }
            finally
            {
        ...
        private void DamageNearestAdditionalMonsters(DefenderUnit source, MonsterUnit primaryTarget, int count, float damage)
        {
            ...
            candidates.Sort((a, b) => Vector3.SqrMagnitude(a.transform.position - origin).CompareTo(Vector3.SqrMagnitude(b.transform.position - origin)));
            int checkedCount = Mathf.Min(Mathf.Max(0, count), candidates.Count);
            for (int i = 0; i < checkedCount; i++)
            {
                candidates[i].TakeDamage(damage, false, source);
            }
        }
        ...
        private void DamageRandomMonsters(DefenderUnit source, int shotCount, float damage)
        {
            ...
            for (int i = 0; i < shotCount && candidates.Count > 0; i++)
            {
        ...
        private void DamageFarthestAdditionalMonsters(DefenderUnit source, MonsterUnit primaryTarget, int count, float damage)
        {
            ...
            candidates.Sort((a, b) => Vector3.SqrMagnitude(b.transform.position - origin).CompareTo(Vector3.SqrMagnitude(a.transform.position - origin)));
            int checkedCount = Mathf.Min(Mathf.Max(0, count), candidates.Count);
            for (int i = 0; i < checkedCount; i++)
            {
                candidates[i].TakeDamage(damage, false, source);
        ...
        private IEnumerator HeroDamageZoneRoutine(DefenderUnit source, SkillDefinition skill, Vector3 center, float radius, float damagePerTick, float duration, float tickInterval)
        {
            ...
            while (elapsed < duration)
            {
                ...
                for (int i = 0; i < monsters.Count; i++)
                {
                    ...
                    {
                        DefenderUnit.RunWithSkillDamageContext(skill, () => monster.TakeDamage(damagePerTick, false, source));
                    }
                }
        ...
        private IEnumerator DelayedRandomHeroDamageZones(DefenderUnit source, SkillDefinition skill, float delay, int count, float radius, float damagePerTick, float duration, float tickInterval)
        {
            ...
            for (int i = 0; i < checkedCount; i++)
            {
                Vector3 center = ResolveRandomMonsterPosition(source, 3.0f + i * 0.45f);
        ...
        private IEnumerator ResolveHero54Retaliation(DefenderUnit source, float duration, float returnRatio, float radius, float shieldShareRatio)
        {
            ...
            float storedDamage = hero54StoredDamage.TryGetValue(source, out float value) ? value : 0f;
        ...
        private void ShowRandomGoldFeedback(AugmentDefinition augment, int reward, int minGold, int maxGold, float triggerChance = 0f)
        {
            ...
            string chanceLabel = triggerChance > 0f
                ? " (" + Mathf.RoundToInt(Mathf.Clamp01(triggerChance) * 100f) + "% 발동)"
                : string.Empty;
            ...
            string detail = triggerChance > 0f
                ? "발동 확률 " + Mathf.RoundToInt(Mathf.Clamp01(triggerChance) * 100f) + "% / 최대 " + safeMaxGold + "G"
                : augment.title + " / 최대 " + safeMaxGold + "G 중 상위 20%";
            RuntimeAudioUtility.PlayJackpotMajor();
            RuntimeGameFeel.ShowJackpotReveal(
                "골드 대박!",
                "JACKPOT",
                "+" + safeReward + "G",
                Color.Lerp(augment.accentColor, new Color(1f, 0.88f, 0.18f), 0.45f),
                detail,
                2.6f);
        }
        ...
        {
            if (reward < GoldJackpotMinimumReward || maxGold <= minGold)
            {
                return false;
            }

            int jackpotThreshold = Mathf.CeilToInt(Mathf.Lerp(minGold, maxGold, GoldJackpotRollThreshold01));
        ...
        private void ShowEconomyBanner(string message, Color color, float duration, float cooldown)
        {
            ...
            if (cooldown > 0f && Time.time < nextEconomyBannerTime)
            {
                return;
            }
        ...
        {
            if (style == AugmentStyle.Stable) return "안정";
            if (style == AugmentStyle.Growth) return "성장";
            if (style == AugmentStyle.Gamble) return "도박";
            return "빌드";
        }
        ...
        private string GetChoiceLabel(AugmentDefinition augment)
        {
            ...
            {
                if (augment.heroTier == HeroAugmentTier.Mythic) return "유닛 전용 · 신화";
                if (augment.heroTier == HeroAugmentTier.Rare) return "유닛 전용 · 희귀";
                return "유닛 전용 · 일반";
            }
        ...
        private void AddTacticalGeneralAugments()
        {
            ...
            AddAugmentIfMissing(CreateAugment(
                "shrapnel_doctrine", "\uc0b0\ud0c4 \uad50\ub9ac",
                "\ubaa8\ub4e0 \ud3c9\ud0c0\uc774 \uc8fc\ubcc0 1.8m\uc5d0 24% \ubc18\uacbd \ud53c\ud574\ub97c \uc90d\ub2c8\ub2e4. \ub2e4\uc218 \uc6e8\uc774\ube0c\uc5d0 \ud2b9\ud654\ub429\ub2c8\ub2e4.",
                AugmentStyle.Stable, AugmentEffectType.BasicAttackSplash, 1.8f, 0.24f, 0f, new Color(1f, 0.46f, 0.34f)));
            AddAugmentIfMissing(CreateAugment(
                "siege_dismantler", "\ubcf4\uc2a4 \ud574\uccb4\ubc18",
                "\ubcf4\uc2a4\uc5d0\uac8c \uc8fc\ub294 \ud53c\ud574 +22%. \ubcf4\uc2a4 \ub77c\uc6b4\ub4dc\uc5d0\uc11c \ud655\uc2e4\ud55c \ucc98\uce58 \uc18d\ub3c4\ub97c \ub9cc\ub4ed\ub2c8\ub2e4.",
                AugmentStyle.Stable, AugmentEffectType.BossDamageMultiplier, 0.22f, 0f, 0f, new Color(1f, 0.42f, 0.28f)));
            AddAugmentIfMissing(CreateAugment(
                "opening_formation", "\uac1c\uc804 \ud3ec\uc9c4",
                "\uac01 \ub77c\uc6b4\ub4dc \uc2dc\uc791 \ud6c4 7\ucd08\ub3d9\uc548 \uacf5\uaca9\ub825 +22%, \uacf5\uaca9 \uc18d\ub3c4 +14%. \ucd08\ubc18 \ud30c\ub3c4\ub97c \ube60\ub974\uac8c \uc815\ub9ac\ud569\ub2c8\ub2e4.",
                AugmentStyle.Growth, AugmentEffectType.RoundStartBurst, 0.22f, 0.14f, 7f, new Color(0.28f, 1f, 0.82f)));
            AddAugmentIfMissing(CreateAugment(
            ...
            AddAugmentIfMissing(CreateAugment(
                "rare_fortress_frame", "\uacac\uace0\ud55c \uc694\uc0c8",
                "\ubaa8\ub4e0 \uc720\ub2db \ucd5c\ub300 \uccb4\ub825 +48%. \ubcf4\uc2a4 \ud328\ud134\uc744 \ubc84\ud2f0\uae30 \uc704\ud55c \ub808\uc5b4 \uc0dd\uc874 \uc99d\uac15\uccb4\uc785\ub2c8\ub2e4.",
                AugmentStyle.Stable, AugmentEffectType.MaxHealthMultiplier, 0.48f, 0f, 0f, new Color(0.24f, 0.62f, 1f), ChoiceRarity.Rare));
            AddAugmentIfMissing(CreateAugment(
                "legend_boss_execution", "\uc885\ub9d0 \uc0ac\ub0e5 \uc9c0\ub839",
                "\ubcf4\uc2a4\uc5d0\uac8c \uc8fc\ub294 \ud53c\ud574 +62%. \uc804\uc124 \ub4f1\uae09\uc758 \ubcf4\uc2a4 \ud5c8\ub4e4 \ub3cc\ud30c \uc120\ud0dd\uc9c0\uc785\ub2c8\ub2e4.",
                AugmentStyle.Stable, AugmentEffectType.BossDamageMultiplier, 0.62f, 0f, 0f, new Color(1f, 0.72f, 0.20f), ChoiceRarity.Legendary));
            AddAugmentIfMissing(CreateAugment(
                "legend_arcane_surge", "\ucd08\uc6d4 \ub9c8\ub098 \uc6a9\uc18c",
                "\uc2a4\ud0ac \uc704\ub825 +55%. \uc804\uc124 \ub4f1\uae09\uc758 \uc2a4\ud0ac \ube4c\ub4dc \uacb0\uc815\ud0c0\uc785\ub2c8\ub2e4.",
                AugmentStyle.Growth, AugmentEffectType.SkillPowerMultiplier, 0.55f, 0f, 0f, new Color(1f, 0.72f, 0.20f), ChoiceRarity.Legendary));
        }
        ...
        private void AddHeroAugmentSet()
        {
            ...
            AddHeroAugment("hero04_contagion_r", "맹독 전염", "일반 적을 처치하면 가장 가까운 적에게 4.5초 동안 0.75초마다 공격력 42%의 독 피해를 줍니다.", "hero_04", HeroAugmentTier.Rare, new Color(0.40f, 0.86f, 0.28f));
            ...
            AddHeroAugment("hero06_storm_lance_m", "폭풍창 재시전", "전방 6.4m에 공격력 125%의 넓은 관통 피해를 주고 스킬을 위력 48%로 한 번 더 시전합니다.", "hero_06", HeroAugmentTier.Mythic, new Color(0.72f, 0.90f, 1f));

            AddHeroAugment("hero07_soul_scythe_n", "영혼 수확", "스킬 대상에게 추가 피해와 2.5초 24% 둔화를 주고, 이 유닛이 3킬할 때마다 마나 12%를 회복합니다.", "hero_07", HeroAugmentTier.Normal, new Color(0.70f, 0.36f, 0.88f));
            AddHeroAugment("hero07_reaper_form_r", "10킬 사신화", "이 유닛이 10킬하면 24초 동안 공격력 +35%, 공격속도 +45%의 사신 상태가 됩니다.", "hero_07", HeroAugmentTier.Rare, new Color(0.62f, 0.20f, 0.82f));
            ...
            AddHeroAugment("hero08_petrify_spread_n", "추가 석화 주시", "스킬의 주 대상이 일반 적이면 2.3초 석화합니다. 보스에게는 공격력 25% 추가 피해를 줍니다.", "hero_08", HeroAugmentTier.Normal, new Color(0.74f, 0.70f, 1f));
            ...
            AddHeroAugment("hero12_chain_cast_m", "연쇄 암살 시전", "55%부터 성공할 때마다 확률이 15%p 감소하며 최대 3회, 위력 58%로 재시전하고 매회 마나 8%를 회복합니다.", "hero_12", HeroAugmentTier.Mythic, new Color(0.98f, 0.70f, 1f));
            ...
            AddHeroAugment("hero21_alpha_pack_m", "강화 미니미 3기", "스킬 사용 시 체력·공격력 55%의 미니미 3기를 소환하고 본체는 5초간 공격력 +12%, 공격속도 +18%를 얻습니다.", "hero_21", HeroAugmentTier.Mythic, new Color(0.70f, 1f, 0.50f));
            ...
            AddHeroAugment("hero23_power_hit_n", "추가 강타", "스킬의 주 대상에게 공격력 75%의 추가 피해를 줍니다.", "hero_23", HeroAugmentTier.Normal, new Color(1f, 0.70f, 0.44f));
            ...
            AddHeroAugment("hero31_iron_growth_m", "강철 전투 성장", "스킬마다 최대 체력 +4%(최대 28%), 6초간 공격력·공격속도 +22%, 피해 감소 35%를 얻습니다.", "hero_31", HeroAugmentTier.Mythic, new Color(0.84f, 0.90f, 1f));

            AddHeroAugment("hero32_predator_rhythm_n", "포식 리듬", "스킬 사용 시 5초 동안 공격력 +10%, 공격속도 +20%를 얻습니다.", "hero_32", HeroAugmentTier.Normal, new Color(0.70f, 0.86f, 0.48f));
            AddHeroAugment("hero32_pack_hunt_r", "무리 사냥탄", "스킬 사용 시 무작위 적에게 공격력 60% 추가 사격 2발을 발사합니다. 같은 적을 다시 맞힐 수 있습니다.", "hero_32", HeroAugmentTier.Rare, new Color(0.58f, 0.80f, 0.40f));
            ...
            AddHeroAugment("hero51_residual_lightning_r", "30% 잔류 재시전", "스킬 사용 시 30% 확률로 위력 45%의 스킬을 한 번 더 시전합니다.", "hero_51", HeroAugmentTier.Rare, new Color(0.36f, 0.60f, 1f));
            ...
            AddHeroAugment("hero52_star_shower_m", "처치 별빛 소나기", "스킬 때 작은 장판 2개를 추가하고, 스킬로 적을 처치하면 처치 위치 주변에 운석 장판 3개를 생성합니다.", "hero_52", HeroAugmentTier.Mythic, new Color(1f, 0.58f, 0.30f));
            ...
            AddHeroAugment("hero55_wall_quake_r", "철벽 충격파", "스킬 착탄 지점 반경 3m에 공격력 90%의 추가 피해를 줍니다.", "hero_55", HeroAugmentTier.Rare, new Color(0.48f, 0.68f, 0.90f));
        ...
        private void AddHeroAugment(string id, string title, string description, string heroId, HeroAugmentTier tier, Color color)
        {
            ...
            if (tier == HeroAugmentTier.Normal)
            {
                style = AugmentStyle.Stable;
            }
            else if (tier == HeroAugmentTier.Rare)
            {
        ...
        private static string ResolveHeroAugmentTargetLabel(string heroId)
        {
            switch (heroId)
            {
                case "hero_01": return "화염";
                case "hero_02": return "치유";
                case "hero_03": return "빙결";
                case "hero_04": return "독";
                case "hero_05": return "방패";
                case "hero_06": return "돌격";
                case "hero_07": return "사신";
                case "hero_08": return "메두사";
                case "hero_09": return "창";
                case "hero_10": return "바람";
                case "hero_11": return "흡수";
                case "hero_12": return "암살";
                case "hero_13": return "배터리";
                case "hero_14": return "빛";
                case "hero_21": return "소환";
                case "hero_22": return "가시방패";
                case "hero_23": return "늑대";
                case "hero_31": return "전투";
                case "hero_32": return "야성";
                case "hero_33": return "감염";
                case "hero_51": return "번개";
                case "hero_52": return "운석";
                case "hero_54": return "가고일";
                case "hero_55": return "아머";
                case "hero_56": return "오토";
                case "hero_57": return "브로큰";
        ...
        {
            augmentPool.Add(CreateAugment("power_core", "화력 코어", "모든 유닛 공격력 +20%. 가장 안정적인 전투력 상승입니다.", AugmentStyle.Stable, AugmentEffectType.AttackPowerMultiplier, 0.20f, 0f, 0f, new Color(1f, 0.57f, 0.28f)));
            augmentPool.Add(CreateAugment("rapid_dice", "속전속결", "모든 유닛 공격속도 +15%. 평타와 마나 수급이 빨라집니다.", AugmentStyle.Stable, AugmentEffectType.AttackSpeedMultiplier, 0.15f, 0f, 0f, new Color(0.45f, 0.90f, 1f)));
            augmentPool.Add(CreateAugment("sharp_eye", "예리한 시선", "치명타 확률 +10%. 크리티컬 빌드의 기본 축입니다.", AugmentStyle.Stable, AugmentEffectType.CriticalChance, 0.10f, 0f, 0f, new Color(1f, 0.86f, 0.32f)));
            ...
            augmentPool.Add(CreateAugment("splash_round", "연쇄 폭발탄", "평타가 주변 몬스터에게 30% 폭발 피해를 줍니다.", AugmentStyle.Stable, AugmentEffectType.BasicAttackSplash, 1.55f, 0.30f, 0f, new Color(1f, 0.35f, 0.48f)));
            ...
            augmentPool.Add(CreateAugment("skill_overload", "스킬 과부하", "스킬 위력 +22%. 마법/소환 빌드에 잘 맞습니다.", AugmentStyle.Stable, AugmentEffectType.SkillPowerMultiplier, 0.22f, 0f, 0f, new Color(0.96f, 0.50f, 1f)));
            ...
            augmentPool.Add(CreateAugment("boss_breaker", "보스 브레이커", "보스에게 주는 피해 +28%. 보스 라운드 안정성이 올라갑니다.", AugmentStyle.Stable, AugmentEffectType.BossDamageMultiplier, 0.28f, 0f, 0f, new Color(1f, 0.36f, 0.30f)));
            augmentPool.Add(CreateAugment("sniper_window", "전군 사거리 보강", "모든 유닛의 기본 공격 대상 탐색 사거리가 0.5m 증가합니다. 투사체 속도와 스킬 범위는 변하지 않습니다.", AugmentStyle.Stable, AugmentEffectType.AttackRangeFlat, 0.5f, 0f, 0f, new Color(0.50f, 0.74f, 1f)));
        ...
        {
            augmentPool.Add(CreateAugment("mining_worker", "골드 광부", "라운드 시작마다 2G + 라운드당 0.30G를 채굴합니다.", AugmentStyle.Growth, AugmentEffectType.WorkerGoldPerRound, 2f, 0.30f, 0f, new Color(1f, 0.78f, 0.22f)));
            ...
            augmentPool.Add(CreateAugment("opening_burst", "개전 신호탄", "라운드 시작 후 5초 동안 공격력 +30%, 공격속도 +18%.", AugmentStyle.Growth, AugmentEffectType.RoundStartBurst, 0.30f, 0.18f, 5f, new Color(0.30f, 1f, 0.86f)));
            augmentPool.Add(CreateAugment("union_mine", "공동 광산", "라운드 시작마다 3G + 라운드당 0.42G를 채굴합니다.", AugmentStyle.Growth, AugmentEffectType.WorkerGoldPerRound, 3f, 0.42f, 0f, new Color(1f, 0.72f, 0.30f)));
            augmentPool.Add(CreateAugment("steady_contract", "안정 계약", "라운드 클리어 보상 +4G. 초반 운영이 안정됩니다.", AugmentStyle.Growth, AugmentEffectType.RoundGoldBonus, 4f, 0f, 0f, new Color(0.70f, 1f, 0.66f)));
            ...
            augmentPool.Add(CreateAugment("hoard_interest", "\uC800\uCD95 \uC774\uC790", "\uC804\uD22C \uC911 100G \uC774\uC0C1 \uBCF4\uC720 \uC2DC 3\uCD08\uB9C8\uB2E4 +2G.", AugmentStyle.Growth, AugmentEffectType.HoardInterestGold, 100f, 2f, 3f, new Color(1f, 0.90f, 0.30f)));
        }
        ...
        private void AddGambleAugments()
        {
            ...
            augmentPool.Add(CreateAugment("pickpocket", "동전 약탈자", "몬스터 처치 시 16% 확률로 1~5G를 추가 획득합니다.", AugmentStyle.Gamble, AugmentEffectType.KillGoldChance, 0.16f, 5f, 0f, new Color(1f, 0.82f, 0.30f)));
            ...
            augmentPool.Add(CreateAugment("boss_bet", "보스 올인", "보스 라운드 클리어 시 24~52G를 획득합니다.", AugmentStyle.Gamble, AugmentEffectType.BossRoundBet, 24f, 28f, 0f, new Color(1f, 0.30f, 0.34f)));
            ...
            augmentPool.Add(CreateAugment("risky_vault", "위험 금고", "즉시 0~78G를 얻습니다. 운이 나쁘면 아무것도 얻지 못합니다.", AugmentStyle.Gamble, AugmentEffectType.RandomInstantGold, 0f, 78f, 0f, new Color(1f, 0.40f, 0.72f)));
            ...
            augmentPool.Add(CreateAugment("dopamine_button", "도파민 버튼", "즉시 1~96G를 얻습니다. 낮게 뜨면 아프지만, 높게 뜨면 판이 열립니다.", AugmentStyle.Gamble, AugmentEffectType.RandomInstantGold, 1f, 96f, 0f, new Color(1f, 0.30f, 0.86f)));
        ...
        private void AddBuildupAugments()
        {
            ...
            augmentPool.Add(CreateAugment("arcane_archive", "비전 기록서", "라운드가 시작될 때마다 스킬 위력이 3.0%씩 누적 증가합니다.", AugmentStyle.Buildup, AugmentEffectType.ScalingSkillPowerPerRound, 0.030f, 0f, 0f, new Color(0.76f, 0.52f, 1f)));
            ...
            augmentPool.Add(CreateAugment("dragon_scale", "보스 사냥 서약", "라운드가 시작될 때마다 보스 피해가 3.0%씩 누적 증가합니다.", AugmentStyle.Buildup, AugmentEffectType.ScalingBossDamagePerRound, 0.030f, 0f, 0f, new Color(1f, 0.42f, 0.30f)));
