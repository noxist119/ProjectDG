...
namespace DefenseGame
{
	...
	public class GamePresentationConfig : ScriptableObject
	{
		...
		public void ApplyToCharacter(CharacterDefinition definition)
		{
			...
			CharacterPresentationOverride entry = characterOverrides.Find((CharacterPresentationOverride candidate) => candidate != null && candidate.characterId == definition.id);
			if (entry == null)
			{
				if (!useRandomCharacterPrefabFallback || characterOverrides.Count != 0 || !TryGetRandomCharacterOverride(out var randomEntry))
				{
					return;
				}
				entry = randomEntry;
		...
		public void ApplyToMonster(MonsterDefinition definition)
		{
			...
			MonsterPresentationOverride entry = monsterOverrides.Find((MonsterPresentationOverride candidate) => candidate != null && candidate.monsterId == definition.id);
			if (entry == null && TryGetOrderedMonsterOverride(definition, out var orderedEntry))
			{
				entry = orderedEntry;
			}
			if (entry != null)
		...
		public int GetHighestConfiguredCharacterIndex()
		{
			...
			for (int i = 0; i < characterOverrides.Count; i++)
			{
				...
				if (entry != null && !(entry.prefab == null) && TryParseIndex(entry.characterId, out var zeroBasedIndex))
				{
		...
		{
			entry = null;
			List<CharacterPresentationOverride> candidates = characterOverrides.FindAll((CharacterPresentationOverride candidate) => candidate != null && candidate.prefab != null);
		...
		{
			return monsterOverrides.Exists((MonsterPresentationOverride candidate) => candidate != null && candidate.useAsRosterEntry && candidate.prefab != null && candidate.threatLevel == threatLevel);
		}

		public List<MonsterPresentationOverride> GetMonsterRosterEntries(MonsterThreatLevel threatLevel)
		{
			return monsterOverrides.FindAll((MonsterPresentationOverride candidate) => candidate != null && candidate.useAsRosterEntry && candidate.prefab != null && candidate.threatLevel == threatLevel);
		}
		...
		private bool TryGetOrderedMonsterOverride(MonsterDefinition definition, out MonsterPresentationOverride entry)
		{
			...
			List<MonsterPresentationOverride> ordered = monsterOverrides.FindAll((MonsterPresentationOverride candidate) => candidate != null && candidate.prefab != null && candidate.threatLevel == definition.threatLevel);
