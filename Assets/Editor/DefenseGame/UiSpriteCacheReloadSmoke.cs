...
namespace DefenseGame.Editor
{
    public static class UiSpriteCacheReloadSmoke
    {
        ...
        private static SessionReport RunSessionCheck(int sessionIndex)
        {
            ...
            Sprite characterSprite = RollRollUiResource.LoadSprite("Minimi/minimi_fire");
            report.characterSpriteLoaded = IsSpriteUsable(characterSprite);
            UnityEngine.Object.DestroyImmediate(characterSprite);
            Sprite characterReloaded = RollRollUiResource.LoadSprite("Minimi/minimi_fire");
            report.characterSpriteRecoveredAfterDestroy = IsSpriteUsable(characterReloaded);

            UiSkinResources skin = AssetDatabase.LoadAssetAtPath<UiSkinResources>(SkinAssetPath);
            report.skinLoaded = skin != null;
            ...
            report.passed = report.characterSpriteLoaded
                && report.characterSpriteRecoveredAfterDestroy
                && report.skinLoaded
                && report.skinImageApplied
                && report.runtimeSlicedSpriteRecoveredAfterDestroy
                && (sessionIndex != 2 || (report.rollRollCacheBeforeLoad == 0 && report.skinSlicedCacheBeforeLoad == 0));
        ...
        private static bool CheckSkinImageApplied(UiSkinResources skin)
        {
            ...
            RuntimeUiSkinUtility.ApplyImageSkin(image, skin, "LobbyBattleButton", true, true);
        ...
        private static bool CheckRuntimeSlicedSpriteRecovery()
        {
            ...
            Sprite first = method.Invoke(null, new object[] { source }) as Sprite;
            bool firstLoaded = IsSpriteUsable(first);
            UnityEngine.Object.DestroyImmediate(first);
            Sprite second = method.Invoke(null, new object[] { source }) as Sprite;
        ...
        private static int GetRollRollCacheCount()
        {
            ...
            return value is System.Collections.IDictionary dictionary ? dictionary.Count : -1;
        }
