...
namespace DefenseGame.Tests
{
    internal sealed class ValidationRunRecorder
    {
        ...
        {
            record = new ValidationRunRecord
            {
                executionId = Guid.NewGuid().ToString("N"),
                unityProcessId = Process.GetCurrentProcess().Id.ToString(),
        ...
        internal void Transition(ValidationRunState next, string reason)
        {
            ...
            record.stateTransitions.Add(new ValidationStateTransition
            {
                from = state.ToString(),
                to = next.ToString(),
            ...
            state = next;
            record.finalState = next.ToString();
        }

        internal void RecordAction(string action, string button, int round)
        {
            record.actualEventSystemClick = true;
            if (string.IsNullOrEmpty(record.firstUiAction))
            {
                record.firstUiAction = button;
                record.firstUiActionRound = round;
            }

            record.actions.Add(new ValidationActionRecord
            {
                index = record.actions.Count + 1,
                realtime = Time.realtimeSinceStartup,
                round = round,
                action = action,
                button = button
        ...
        internal void RecordSnapshot(Component controller, string phase)
        {
            if (controller != null)
            {
                record.rounds.Add(RuntimeGameView.CaptureSnapshot(controller, phase));
            }
        }
        ...
        internal void HandleLog(string condition, string stackTrace, LogType type)
        {
            ...
            if (!string.IsNullOrEmpty(condition) && condition.IndexOf("BoardSlot", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                boardSlotWarnings++;
            }
        }

        internal void MarkFailure(string status, string reason)
        {
            record.status = status;
            record.failureReason = reason;
            Transition(ValidationRunState.Failed, reason);
        }
        ...
        internal void FinalizeRecord(Component controller, string status, string reason)
        {
            ...
            {
                record.blockingChoiceReason = "controller_missing";
            }
            ...
            Transition(ValidationRunState.Record, status);
            Transition(ValidationRunState.Finished, status);
        }

        internal string Save()
        {
            string root = ResolveProjectRoot();
            string directory = Path.Combine(root, "BatchPlaytestResults");
            Directory.CreateDirectory(directory);
            string path = Path.Combine(directory, "DefenseGame_Pass21_" + record.strategy + "_Seed" + record.requestedSeed + ".json");
        ...
        private static string ResolveProjectRoot()
        {
            ...
            {
                string candidate = starts[i];
                if (string.IsNullOrEmpty(candidate))
                {
                    continue;
                }

                DirectoryInfo directory = new DirectoryInfo(Path.GetFullPath(candidate));
